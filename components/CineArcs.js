...
function ArcCard({ arc, progress, onOpen, rating }) {
  ...
  const stageLabel = meta.stages?.[0] || 'Warm-up';
  ...
  return (
    ...
    >
      {/* COVER — dominant image with mini-gallery overlay */}
      <div style={{ position: 'relative', width: '100%', aspectRatio: '4/3' }}>
        {/* Main cover */}
        <div style={{
          position: 'absolute', inset: 0,
          background: cover ? `url(${cover})` : `linear-gradient(135deg, ${accent}30, ${T.surface})`,
          backgroundSize: 'cover', backgroundPosition: 'center',
        }}/>
        {/* Gradient overlay */}
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, rgba(6,6,11,0.1) 0%, rgba(6,6,11,0.75) 100%)' }}/>
        {/* Genre badge */}
        <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', gap: 5 }}>
          <span style={{ fontSize: 9, fontWeight: 800, color: accent, background: `${accent}22`, border: `1px solid ${accent}55`, borderRadius: 20, padding: '3px 8px', letterSpacing: 1, textTransform: 'uppercase' }}>
            {genreLabel}{mediaLabel ? ` · ${mediaLabel}` : ''}
          </span>
        </div>
        {/* Title count */}
        <div style={{ position: 'absolute', top: 10, right: 10 }}>
          <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.5)' }}>{total} titles</span>
        </div>
        {/* Mini poster gallery — 4 numbered thumbnails at bottom */}
        <div style={{ position: 'absolute', bottom: 10, left: 10, display: 'flex', gap: 5 }}>
          {(posters.length ? posters : [null, null, null, null]).slice(0, 4).map((src, i) => (
            <div key={i} style={{ position: 'relative', width: 34, height: 48, borderRadius: 7, overflow: 'hidden', background: T.surface, border: `1px solid rgba(255,255,255,0.12)`, flexShrink: 0 }}>
              {src && <img src={src} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>}
              <div style={{ position: 'absolute', bottom: 2, left: 2, width: 14, height: 14, borderRadius: '50%', background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: 7, fontWeight: 800, color: '#fff' }}>{i + 1}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        <div style={{ fontFamily: T.serif, fontSize: 14, fontWeight: 800, fontStyle: 'italic', color: T.text, lineHeight: 1.25, marginBottom: 5 }}>
          {arc.title}
        </div>
        {arc.summary && (
          <div style={{ fontSize: 11, color: T.text3, lineHeight: 1.45, marginBottom: 9, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
            {arc.summary}
          </div>
        )}
        {/* Progress bar */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
          <span style={{ fontSize: 10, fontWeight: 700, color: accent }}>{stageLabel}</span>
          <Stars value={typeof rating === 'number' ? rating : 0} size={10} color={accent}/>
        </div>
        <div style={{ height: 3, borderRadius: 3, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${pct}%`, background: `linear-gradient(90deg, ${accent}88, ${accent})`, borderRadius: 3, transition: 'width 0.4s ease' }}/>
        </div>
        <div style={{ fontSize: 9.5, color: T.text3, marginTop: 4 }}>{watched}/{total} · {pct}%</div>
      </div>
    </button>
  );
}
