...
function ArcCard({ arc, progress, onOpen, rating }) {
  ...
  while (posters.length < 3 && arc.cover_poster) {
    // pad with cover if API only returned one
    if (!posters.includes(arc.cover_poster)) posters.push(arc.cover_poster);
    else break;
  }
  const summary = arc.summary || arc.subtitle || '';
  ...
  return (
    ...
    >
      {/* Top: stacked mini posters like a vertical path */}
      <div
        style={{
          display: 'flex',
          gap: 6,
        ...
        {(posters.length ? posters : [null, null, null, null]).slice(0, 4).map((src, i) => (
          <div
            ...
            style={{
              ...
              border: `1px solid ${T.hairline}`,
              position: 'relative',
              boxShadow: i === posters.length - 1 || i === 3 ? `0 0 0 1px ${accent}33` : 'none',
            }}
          >
            {src ? (
              <img src={src} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
            ) : (
              <div style={{ width: '100%', height: '100%', background: `linear-gradient(160deg, ${accent}22, ${T.surface})` }} />
            )}
            <div
              style={{
                position: 'absolute',
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        ...
        {/* Progress + rating footer — home-card style strip */}
        <div
          style={{
            marginTop: 12,
            paddingTop: 12,
            ...
            >
              <div
                style={{
                  ...
                  background: accent,
                  borderRadius: 4,
                  boxShadow: pct ? `0 0 8px ${accent}55` : 'none',
                }}
              />
            </div>
          </div>
          <div style={{ textAlign: 'right', flexShrink: 0 }}>
            <Stars value={rating?.avg || 0} size={12} color={accent} />
            <div style={{ fontSize: 10, color: T.text3, marginTop: 2 }}>
              {rating?.avg != null ? Number(rating.avg).toFixed(1) : 'Rate'}
            </div>
          </div>
        </div>
      </div>
