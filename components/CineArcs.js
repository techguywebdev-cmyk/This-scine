 ...
 *   {showArcs && <CineArcs accent={accent} onClose={() => setShowArcs(false)} onWatchTrailer={...} watchedIds={...} onToggleWatched={...} />}
 */
...
export default function CineArcs({ onClose, accent = '#F5A623', onWatchTrailer }) {
      ...
      {!detail ? (
        <>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 18px 28px', WebkitOverflowScrolling: 'touch' }}>
            ...
            >
              <div style={{ fontSize: 13, fontWeight: 700, color: T.text }}>Your watchlist, as an Arc</div>
              <div style={{ fontSize: 12, color: T.text3, marginTop: 6, lineHeight: 1.45 }}>
                Coming next: turn any personal watchlist into a mild→peak path with the same progress meter.
              </div>
            </div>
          </div>
        </>
