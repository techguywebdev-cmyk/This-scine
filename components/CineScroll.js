...
function MovieCard({movie,isActive,index,onFindSimilar,onAuthRequired,onSave,isSaved,onTrailer}){
  ...
  const[liked,setLiked]=useState(false);const[userRating,setUserRating]=useState(0);const[showComments,setShowComments]=useState(false);const[showStars,setShowStars]=useState(false);const[hoverStar,setHoverStar]=useState(0);const[imgLoaded,setImgLoaded]=useState(false);const[likeCount]=useState(Math.floor(Math.random()*60+8)*100);const[showHint,setShowHint]=useState(false);const[showAddToList,setShowAddToList]=useState(false);
      ...
      <div style={{minHeight:'100%',background:T.bg,paddingBottom:48}}>
        ...
        {!loading&&list&&(
          <div style={{padding:'0 20px'}}>
            ...
            {/* MOVIE LIST */}
            <Eyebrow color={T.text3} style={{marginBottom:12}}>{list.movie_count} {list.movie_count===1?'Film':'Films'}</Eyebrow>
            {movies.length===0?(
              ...
              <div style={{display:'flex',flexDirection:'column'}}>
                {movies.map((m,i)=>(
                  <div key={m.movie_id} style={{display:'flex',gap:12,padding:'13px 0',borderTop:i>0?`1px solid ${T.hairline}`:'none',alignItems:'flex-start'}}>
                    <button onClick={()=>onWatchTrailer&&onWatchTrailer({id:m.movie_id,title:m.movie_title,poster:m.movie_poster,year:m.movie_year,rating:m.movie_rating,accent:m.movie_accent||accentColor,isTV:m.is_tv,mediaType:m.is_tv?'tv':'movie'})}
...
function ListsScreen({onClose,accent,onWatchTrailer,onSave,watchlistIds}){
  const{isSignedIn}=useUser();
  ...
  return(
    <div style={{position:'fixed',inset:0,zIndex:120,background:T.bg,display:'flex',flexDirection:'column',animation:'playerSlideUp 0.38s cubic-bezier(0.22,1,0.36,1)'}}>
      ...
      {selectedList&&<ListDetailSheet listId={selectedList} onClose={()=>setSelectedList(null)} accent={accent} onWatchTrailer={onWatchTrailer} onSave={onSave} watchlistIds={watchlistIds}/>}
      ...
      <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none',padding:'16px 20px'}}>
        {loading?(
          ...
          <div style={{display:'flex',flexDirection:'column',gap:1,background:T.hairline,borderRadius:18,overflow:'hidden'}}>
            {lists.map((list,i)=>(
              <button key={list.id} onClick={()=>setSelectedList(list.id)} style={{display:'flex',gap:14,alignItems:'center',background:T.bg,border:'none',padding:'14px 16px',cursor:'pointer',textAlign:'left',fontFamily:'inherit',width:'100%'}}>
                {/* COVER THUMBNAIL */}
...
export default function CineScroll(){
  ...
  const{openSignIn}=useClerk();
  const[movies,setMovies]=useState([]);const[loading,setLoading]=useState(true);const[activeIndex,setActiveIndex]=useState(0);const[activeGenre,setActiveGenre]=useState('');const[activeMood,setActiveMood]=useState('Trending');const[showFilter,setShowFilter]=useState(false);const[showAuth,setShowAuth]=useState(false);const[showProfile,setShowProfile]=useState(false);const[showLists,setShowLists]=useState(false);const[showFriends,setShowFriends]=useState(false);const[trailerMovie,setTrailerMovie]=useState(null);const[similarMovie,setSimilarMovie]=useState(null);const[watchlistIds,setWatchlistIds]=useState(new Set());const[watchlist,setWatchlist]=useState([]);const[userReviews,setUserReviews]=useState([]);const[loadingProfileData,setLoadingProfileData]=useState(false);
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      {showLists&&<ListsScreen onClose={()=>setShowLists(false)} accent={accent} onWatchTrailer={setTrailerMovie} onSave={handleSave} watchlistIds={watchlistIds}/>}
