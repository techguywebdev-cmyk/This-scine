...
// ─── MESSAGES INBOX ─────────────────────────────────────────────────────────
function MessagesInbox({ onClose, accent, onOpenChat }) {
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${T.hairline}`, flexShrink: 0 }}>
          <div>
            <div style={{ fontFamily: T.serif, fontSize: 22, fontWeight: 800, fontStyle: 'italic', color: T.text }}>Messages</div>
            <div style={{ fontSize: 11, color: T.text3, marginTop: 2 }}>Requests & conversations</div>
          </div>
          <button
            type="button"
            onClick={onClose}
          ...
          >
            <SvgIcon name="close" size={14} color={T.text2} />
          </button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', padding: '8px 0' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '48px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
              <SvgIcon name="chat" size={30} color={T.hairlineStrong} />
              <div style={{ fontSize: 15, fontWeight: 700, color: T.text, fontFamily: T.serif, fontStyle: 'italic' }}>No messages yet</div>
              <div style={{ fontSize: 12.5, color: T.text3, lineHeight: 1.5, maxWidth: 260 }}>
                Open a friend’s profile and tap the chat icon to start a conversation.
              </div>
            </div>
          ) : (
            [...conversations]
              .sort((a, b) => {
                const au = a.unread || a.unread_count > 0 ? 1 : 0;
                const bu = b.unread || b.unread_count > 0 ? 1 : 0;
                if (au !== bu) return bu - au;
                return new Date(b.last_at || 0) - new Date(a.last_at || 0);
              })
              .map((c, i) => {
              const isUnread = !!(c.unread || (c.unread_count > 0));
              return (
              <button
                key={c.peer_id || i}
                type="button"
                ...
                style={{
                  ...
                  background: isUnread ? 'rgba(255,255,255,0.07)' : 'transparent',
                  border: 'none',
              ...
              >
                <div
                  style={{
                    ...
                    overflow: 'hidden',
                    background: `${accent}18`,
                    border: `1px solid ${isUnread ? accent + '66' : accent + '30'}`,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    flexShrink: 0,
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                    <span
                      style={{
                        ...
                        color: isUnread ? T.text : 'rgba(255,255,255,0.55)',
                        fontFamily: T.serif,
                        fontStyle: 'italic',
                      }}
                    >
                    ...
                    <span style={{ fontSize: 10.5, color: isUnread ? accent : T.text3, flexShrink: 0, fontWeight: isUnread ? 700 : 400 }}>{timeAgo(c.last_at)}</span>
                  </div>
                  <div
                    style={{
                      fontSize: 12.5,
                      color: isUnread ? 'rgba(255,255,255,0.88)' : T.text3,
                  ...
                  >
                    ...
                    {isUnread && (
                      ...
                      >
                        {(c.unread_count || 1) > 9 ? '9+' : (c.unread_count || 1)}
                      </span>
                    )}
                  </div>
                </div>
...
function ChatWidget({ peer, onClose, accent }) {
  ...
  const startEdit = (m) => {
    if (!m.from_me || m.msg_type === 'voice' || m.msg_type === 'gif' || m.msg_type === 'sticker') return;
    setEditingId(m.id);
  ...
  const saveEdit = async () => {
    ...
    if (text.length > 1000) { setError('Message too long'); return; }
    setSavingEdit(true);
    const prev = messages.find((m) => m.id === editingId);
    setMessages((p) => p.map((m) => (m.id === editingId ? { ...m, text, edited: true, edited_at: new Date().toISOString() } : m)));
    try {
      const res = await fetch('/api/messages', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) {
        if (prev) setMessages((p) => p.map((m) => (m.id === editingId ? prev : m)));
        setError(data.error || 'Could not edit');
      } else if (data.message) {
        setMessages((p) => p.map((m) => (m.id === editingId ? { ...m, ...data.message } : m)));
      }
    } catch {
      if (prev) setMessages((p) => p.map((m) => (m.id === editingId ? prev : m)));
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: T.text, fontFamily: T.serif, fontStyle: 'italic' }}>{name}</div>
            <div style={{ fontSize: 11, color: T.text3 }}>{handle || 'Direct message'}</div>
          </div>
          <button
            type="button"
            onClick={onClose}
              ...
              return (
                ...
                >
                  {isVoice && m.media_url ? (
                    <div
                      style={{
                        background: m.from_me ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.06)',
                    ...
                    editingId === m.id ? (
                      <div style={{ width: '100%', minWidth: 180 }}>
                        ...
                        />
                        <div style={{ display: 'flex', gap: 8, marginTop: 6, justifyContent: m.from_me ? 'flex-end' : 'flex-start' }}>
                          <button type="button" onClick={cancelEdit} style={{ background: 'transparent', border: 'none', color: T.text3, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Cancel</button>
                          <button type="button" onClick={saveEdit} disabled={savingEdit || !editDraft.trim()} style={{ background: accent, border: 'none', borderRadius: 12, padding: '6px 12px', color: '#07070F', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: savingEdit || !editDraft.trim() ? 0.5 : 1 }}>Save</button>
                        </div>
                      </div>
                    ) : (
                    <div
                      onClick={() => m.from_me && startEdit(m)}
                      style={{
                        background: m.from_me ? accent : T.surface2,
                        color: m.from_me ? '#07070F' : T.text,
                        borderRadius: m.from_me ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                        padding: '10px 13px',
                        fontSize: 13.5,
                        lineHeight: 1.45,
                        ...
                        cursor: m.from_me ? 'pointer' : 'default',
                      }}
                    >
                      {m.text}
                    </div>
                  ...
                  >
                    ...
                    <span style={{ color: m.from_me && m.read ? accent : T.text3 }}>
                      {statusLabel(m)}
                      {(m.edited || m.edited_at) ? ' · Edited' : ''}
                    </span>
                    {m.from_me && !isVoice && !isGif && !isSticker && editingId !== m.id && (
                      <button
                        type="button"
                        onClick={() => startEdit(m)}
                        style={{ background: 'none', border: 'none', color: T.text3, fontSize: 10, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', padding: 0, marginLeft: 2 }}
                      >
                        Edit
                      </button>
                    )}
...
function FriendsScreen({ onClose, accent, onWatchTrailer, onAddToWatchlist }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 90, background: T.bg, display: 'flex', flexDirection: 'column', animation: 'playerSlideUp 0.4s cubic-bezier(0.22,1,0.36,1)' }}>
      ...
      {toast && <Toast message={toast} accent={accent} />}
      {showNotifs && <NotificationsPanel onClose={() => setShowNotifs(false)} accent={accent} notifications={notifications} loading={loadingNotifs} onMarkRead={handleMarkRead} onFollowBack={handleFollowBack} onOpenChat={(p)=>{ setReturnToInbox(false); setChatPeer(p); }} />}
      {chatPeer && <ChatWidget peer={chatPeer} onClose={() => { setChatPeer(null); if (returnToInbox) { setShowMessages(true); setReturnToInbox(false); } }} accent={accent} />}
      {showMessages && (
        <MessagesInbox
          onClose={() => setShowMessages(false)}
          accent={accent}
          onOpenChat={(p) => { setShowMessages(false); setReturnToInbox(true); setChatPeer(p); }}
        />
      )}
      {followListType && (
        <FollowListModal
