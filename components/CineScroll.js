...
function StreamingBadges({ movieId, mediaType, title, year }) {
  ...
  const openProvider = (p, e) => {
    ...
    // Try app scheme first; if the app isn't installed the browser stays put and we fall back to HTTPS.
    // Universal HTTPS links also hand off to the app on many phones.
    if (app && app !== web) {
      ...
      setTimeout(() => {
        ...
        // If still on page shortly after, open web (or universal) link
