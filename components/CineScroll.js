...
function UserProfileSheet({userId,onClose,accent,onWatchTrailer}){
  ...
  return(
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:108,background:'rgba(0,0,0,0.7)',backdropFilter:'blur(10px)',display:'flex',alignItems:'flex-end',animation:'fadeIn 0.2s ease'}}>
      {toast&&<Toast message={toast} accent={accent}/>}
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxHeight:'85vh',background:'rgba(8,8,16,0.99)',borderRadius:'24px 24px 0 0',border:'1px solid rgba(255,255,255,0.08)',borderBottom:'none',display:'flex',flexDirection:'column',animation:'sheetUp 0.32s cubic-bezier(0.22,1,0.36,1)'}}>
        <style>{`@keyframes sheetUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
        <div style={{width:34,height:4,borderRadius:2,background:'rgba(255,255,255,0.1)',margin:'12px auto 0',flexShrink:0}}/>
        <button onClick={onClose} style={{position:'absolute',top:14,right:14,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'50%',width:30,height:30,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="close" size={13} color="rgba(255,255,255,0.5)"/></button>
        ...
        {loading?(
          ...
          <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none',padding:'20px 20px 32px'}}>
            {/* Header */}
            <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:18}}>
              <div style={{width:64,height:64,borderRadius:'50%',background:`${accentColor}20`,border:`1px solid ${accentColor}40`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:24,fontWeight:700,color:accentColor,flexShrink:0,overflow:'hidden'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:18,fontWeight:800,color:'#fff',fontFamily:"'Playfair Display',serif",fontStyle:'italic'}}>{profile.display_name||profile.username}</div>
                <div style={{fontSize:12,color:'rgba(255,255,255,0.35)',marginTop:2}}>@{profile.username}</div>
              </div>
              {!profile.isSelf&&(
                <button onClick={handleFollow} style={{background:following?'rgba(255,255,255,0.06)':accentColor,border:`1px solid ${following?'rgba(255,255,255,0.1)':accentColor}`,borderRadius:20,padding:'8px 18px',cursor:'pointer',fontSize:12,color:following?'rgba(255,255,255,0.5)':'#07070F',fontFamily:'inherit',fontWeight:700,flexShrink:0,transition:'all 0.2s ease'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:18}}>
              {[{label:'Followers',value:profile.followers},{label:'Following',value:profile.following},{label:'Watchlist',value:profile.watchlistCount}].map(s=>(
            ...
            {/* Watchlist */}
            <div>
              <div style={{fontSize:10,letterSpacing:2,color:'rgba(255,255,255,0.25)',fontWeight:700,textTransform:'uppercase',marginBottom:10}}>
                {profile.isSelf?'Your Watchlist':`${profile.display_name||profile.username}'s Watchlist`}
              </div>
