...
function ProfileSheet({onClose,accent,watchlist,setWatchlist,userReviews,loadingData}){
  ...
  useEffect(()=>{
    fetch('/api/settings').then(r=>r.json()).then(d=>{setWatchlistPublic(d.watchlist_public!==false);setLoadingSettings(false);}).catch(()=>setLoadingSettings(false));
  },[]);
...
function UserProfileSheet({userId,onClose,accent,onWatchTrailer}){
  ...
  // Randomized cover photo: backdrop from a popular movie in the user's #1 genre
  useEffect(()=>{
    if(!profile||!TMDB_KEY)return;
    ...
    fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_KEY}&with_genres=${genreId}&sort_by=popularity.desc&page=1`)
      ...
      .then(d=>{
        ...
        if(results.length>0){
          ...
          setCoverImg(`https://image.tmdb.org/t/p/w1280${pick.backdrop_path}`);
        }
      }).catch(()=>{});
  },[profile,TMDB_KEY]);
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxHeight:'90vh',background:'rgba(8,8,16,0.99)',borderRadius:'24px 24px 0 0',border:'1px solid rgba(255,255,255,0.08)',borderBottom:'none',display:'flex',flexDirection:'column',animation:'sheetUp 0.32s cubic-bezier(0.22,1,0.36,1)',overflow:'hidden'}}>
        ...
        {loading?(
          ...
          <div className="profile-scroll" style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
            {/* COVER PHOTO */}
            <div style={{position:'relative',height:150,flexShrink:0,background:coverImg?`url(${coverImg})`:`linear-gradient(135deg,${accentColor}30,#0a0a14)`,backgroundSize:'cover',backgroundPosition:'center 30%'}}>
              <div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,rgba(8,8,16,0.1) 0%,rgba(8,8,16,0.55) 60%,rgba(8,8,16,0.97) 100%)'}}/>
              ...
              <button onClick={onClose} style={{position:'absolute',top:14,right:14,background:'rgba(0,0,0,0.4)',backdropFilter:'blur(8px)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'50%',width:30,height:30,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="close" size={13} color="rgba(255,255,255,0.8)"/></button>
            </div>

            <div style={{padding:'0 20px 32px',marginTop:-38}}>
              {/* HEADER: avatar + name + follow */}
              <div style={{display:'flex',alignItems:'flex-end',gap:14,marginBottom:16}}>
                <div style={{width:76,height:76,borderRadius:'50%',background:`${accentColor}25`,border:'3px solid #08080F',display:'flex',alignItems:'center',justifyContent:'center',fontSize:28,fontWeight:700,color:accentColor,flexShrink:0,overflow:'hidden'}}>
                ...
                {!profile.isSelf&&(
                  <button onClick={handleFollow} style={{background:following?'rgba(255,255,255,0.06)':accentColor,border:`1px solid ${following?'rgba(255,255,255,0.1)':accentColor}`,borderRadius:20,padding:'9px 20px',cursor:'pointer',fontSize:12,color:following?'rgba(255,255,255,0.5)':'#07070F',fontFamily:'inherit',fontWeight:700,flexShrink:0,transition:'all 0.2s ease',marginBottom:4}}>
