...
function ChatWidget({ peer, onClose, accent }) {
  ...
  const ensurePc = (withVideo) => {
    ...
    pc.onicecandidate = (e) => {
      if (e.candidate) {
        sendSignal({ kind: 'ice', candidate: e.candidate });
      }
    };
    pc.ontrack = (e) => {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (st === 'failed' || st === 'closed' || st === 'disconnected') {
        // don't auto cleanup on brief disconnect
      }
    };
    pcRef.current = pc;
    return pc;
  ...
  const startCall = async (mode) => {
    ...
    try {
      ...
      const pc = ensurePc(mode === 'video');
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);
      await sendSignal({ kind: 'offer', sdp: offer, callType: mode });
    } catch (err) {
      console.error(err);
      setError(err?.message?.includes('Permission') ? 'Camera/mic permission needed' : 'Could not start call');
      cleanupCall();
    }
  };
  ...
  const handleSignal = async (signal, fromMe) => {
    ...
    if (fromMe) return; // ignore own signals except we already sent them

    if (signal.kind === 'offer') {
      if (callMode) return; // already in a call
      setIncomingCall({ type: signal.callType || 'audio', sdp: signal.sdp, callType: signal.callType || 'audio' });
      setCallStatus('incoming');
      return;
    }
    if (signal.kind === 'answer' && pcRef.current) {
      try {
    ...
    if (signal.kind === 'ice' && pcRef.current && signal.candidate) {
      try {
    ...
    if (signal.kind === 'upgrade_offer' && pcRef.current) {
      try {
        await pcRef.current.setRemoteDescription(signal.sdp);
        // enable local video for upgrade
        const vStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'user' }, audio: false });
        const vTrack = vStream.getVideoTracks()[0];
        const existing = pcRef.current.getSenders().find((s) => s.track && s.track.kind === 'video');
        ...
        if (localStreamRef.current) {
          ...
          vStream.getTracks().forEach((tr) => localStreamRef.current.addTrack(tr));
        } else {
        ...
        await sendSignal({ kind: 'upgrade_answer', sdp: answer });
        setCallMode('video');
        setUpgradePrompt(false);
        setCamOff(false);
      } catch (e) {
        console.error('upgrade answer failed', e);
      }
      return;
    }
    if (signal.kind === 'upgrade_answer' && pcRef.current) {
      try {
        if (!pcRef.current.currentRemoteDescription || pcRef.current.signalingState !== 'stable') {
          await pcRef.current.setRemoteDescription(signal.sdp);
        } else {
          await pcRef.current.setRemoteDescription(signal.sdp);
        }
        ...
        console.error('upgrade answer apply failed', e);
      }
      return;
    }
  ...
  const acceptVideoUpgrade = async () => {
    ...
    try {
      ...
      await sendSignal({ kind: 'upgrade_offer', sdp: offer });
      setCallMode('video');
      setCamOff(false);
    } catch (e) {
      console.error(e);
  ...
  // poll signals faster while in chat (and process call_signal msgs)
  useEffect(() => {
    if (!peerId) return;
    let cancelled = false;
    const tick = async () => {
      try {
        ...
        // update visible messages (exclude signals)
        if (!cancelled) {
          const visible = [];
          for (const m of list) {
            ...
            // Show only offer/end as soft system lines (never ICE / raw JSON)
            try {
              const payload = JSON.parse(m.text || '{}');
    ...
    tick();
    const interval = setInterval(tick, callMode || callStatus === 'incoming' ? 900 : 4000);
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    return () => {
      try {
        if (pcRef.current) sendSignal({ kind: 'end' });
      } catch {}
      cleanupCall();
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
        ...
        {callMode && (
          <div
            style={{
              ...
              background:
                callMode === 'audio'
                  ? `radial-gradient(ellipse at 50% 30%, ${accent}55 0%, ${accent}22 35%, #0B0B10 70%)`
                  : '#050508',
            }}
          >
            {/* Top bar */}
                  ...
                  <div
                    style={{
                      ...
                      background: remoteSpeaking
                        ? `radial-gradient(circle, ${accent} 0%, ${accent}88 40%, transparent 70%)`
                        : 'transparent',
                      boxShadow: remoteSpeaking
                        ? `0 0 0 6px ${accent}33, 0 0 40px ${accent}66, 0 0 80px ${accent}33`
                        : `0 0 0 0 transparent`,
            ...
            >
              ...
              {callMode === 'video' ? (
                ...
                >
                  <SvgIcon name="video" size={20} color={camOff ? '#0A0A0F' : '#fff'} />
                </button>
              ) : (
                <button
                  type="button"
                ...
                >
                  <SvgIcon name="video" size={20} color="#fff" />
                </button>
              )}
          ...
          >
            <button
              ...
              style={{
                ...
                height: 40,
                borderRadius: '50%',
                background: showStickers ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,0.06)',
                border: '1px solid rgba(255,255,255,0.08)',
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
            ...
            >
              <SvgIcon name="plus" size={18} color="rgba(255,255,255,0.8)" />
            </button>
            <div
              style={{
                flex: 1,
