...
export function InlinePlayer({ movie, onClose, accent, onSave, isSaved, initialTab, highlightCommentId }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:260,background:T.bg,display:'flex',flexDirection:'column',animation:'playerSlideUp 0.4s cubic-bezier(0.22,1,0.36,1)',overflowY:'auto',WebkitOverflowScrolling:'touch'}}>
          ...
          {tab==='profile'&&(
            <div style={{padding:'16px'}}>
              ...
              <div style={{marginTop:18,marginBottom:8}}>
                <Eyebrow style={{marginBottom:10}}>Notifications</Eyebrow>
                <div style={{fontSize:11.5,color:T.text3,lineHeight:1.45,marginBottom:12}}>
                  Choose how we reach you — email, this browser, and the app when it ships.
                </div>
                {[
                  {key:'email',label:'Email',desc:'Messages & follows to your inbox'},
                  {key:'web',label:'Web push',desc:hasWebPush?'Enabled on this browser':'Browser alerts when you\'re away'},
                  {key:'app',label:'App (coming soon)',desc:'Native push on iOS & Android'},
                ].map(row=>(
                  <div key={row.key} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0',borderTop:`1px solid ${T.hairline}`}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:13,fontWeight:700,color:T.text}}>{row.label}</div>
                      <div style={{fontSize:11,color:T.text3,marginTop:2}}>{row.desc}</div>
                    </div>
                    {row.key==='web'&&!hasWebPush?(
                      ...
                      <span style={{fontSize:10,color:T.text3,fontWeight:600,flexShrink:0}}>Soon</span>
                    ):(
                      <button type="button" onClick={()=>toggleNotify(row.key)} disabled={loadingSettings} role="switch" aria-checked={!!notifyPrefs[row.key]} style={{width:44,height:26,borderRadius:13,border:'none',cursor:'pointer',background:notifyPrefs[row.key]?accent:'rgba(255,255,255,0.12)',position:'relative',flexShrink:0,padding:0}}>
                        <div style={{position:'absolute',top:3,left:notifyPrefs[row.key]?23:3,width:20,height:20,borderRadius:'50%',background:'#fff',transition:'left 0.2s ease'}}/>
                      </button>
                    )}
                  </div>
                ))}
                <div style={{marginTop:8,paddingTop:8,borderTop:`1px solid ${T.hairline}`}}>
                  <div style={{fontSize:11,color:T.text3,marginBottom:8,fontWeight:600}}>Notify me about</div>
                  {[
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                    ...
                    <span style={{ fontSize: 10.5, color: T.text3, flexShrink: 0 }}>{timeAgo(c.last_at)}</span>
                  </div>
                  <div
                    style={{
                      fontSize: 12.5,
                      color: c.unread ? T.text2 : T.text3,
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch('/api/messages', {
        ...
        body: JSON.stringify({ toUserId: peerId, text }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: T.text3 }}>{handle || 'Message request'}</div>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        >
          <div style={{ fontSize: 11.5, color: T.text3, lineHeight: 1.45 }}>
            Send a message as a request. They’ll see it in notifications and can reply here.
          </div>
        </div>
        ...
        >
          {loading && messages.length === 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: 28 }}>
              <div style={{ fontSize: 13, color: T.text3, marginBottom: 8 }}>{error}</div>
              <button
                type="button"
                onClick={load}
                style={{
            ...
            <div style={{ textAlign: 'center', padding: '36px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
              <SvgIcon name="chat" size={28} color={T.hairlineStrong} />
              <div style={{ fontSize: 14, fontWeight: 700, color: T.text, fontFamily: T.serif, fontStyle: 'italic' }}>Say hello</div>
              <div style={{ fontSize: 12.5, color: T.text3, lineHeight: 1.5, maxWidth: 260 }}>
                Your first message is sent as a request. Keep it friendly — movie recs work great.
              </div>
            </div>
          ) : (
            messages.map((m) => (
              ...
              >
                ...
                <div style={{ fontSize: 10, color: T.text3, marginTop: 4, padding: '0 4px' }}>{timeAgo(m.created_at)}</div>
              </div>
            ))
          )}
        </div>
        ...
        >
          <input
            ...
            placeholder={`Message ${handle || name}…`}
            maxLength={1000}
            style={{
              flex: 1,
              background: T.surface2,
          ...
          <button
            ...
            style={{
              background: input.trim() ? accent : T.surface2,
              border: 'none',
              borderRadius: 18,
              width: 44,
              height: 44,
          ...
          >
            <SvgIcon name="send" size={16} color={input.trim() ? '#07070F' : T.text3} filled />
          </button>
        </div>
      </div>
    </>
