  ...
  const load = () => {
    ...
    fetch(`/api/messages?with=${peerId}`)
      ...
      .then((d) => {
        ...
        const list = (d.messages || []).filter((m) => m.msg_type !== 'call_signal');
        setMessages(list);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (!cancelled) {
          const visible = list.filter((m) => m.msg_type !== 'call_signal');
          setMessages(visible);
          if (d.peer) setPeerInfo((p) => ({ ...p, ...d.peer }));
        }
        for (const m of list) {
          if (m.msg_type !== 'call_signal') continue;
          if (processedSignalsRef.current.has(m.id)) continue;
  ...
  return (
    <>
      ...
      >
        ...
        {/* Full-screen active call (WhatsApp-style) */}
        {callMode && (
          <div
            style={{
              position: 'fixed',
          ...
          >
            ...
            <div style={{ flex: 1, position: 'relative', background: '#0B0B10', overflow: 'hidden' }}>
              {callMode === 'video' ? (
                <video
                  ref={remoteVideoRef}
                  autoPlay
                ...
                <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                  <div
                    style={{
                      ...
                      alignItems: 'center',
                      justifyContent: 'center',
                      boxShadow: `0 0 48px ${accent}33`,
                    }}
                  >
                    {peerInfo?.avatar_url ? (
                      <img src={peerInfo.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    ) : (
                      <span style={{ fontFamily: T.serif, fontSize: 44, fontWeight: 700, color: accent }}>{(name || 'U')[0].toUpperCase()}</span>
                    )}
                  </div>
                </div>
              )}
              ...
              >
                <div style={{ fontFamily: T.serif, fontSize: 20, fontWeight: 700, fontStyle: 'italic', color: '#fff' }}>{name}</div>
            ...
            >
              ...
              >
                <SvgIcon name="mic" size={22} color="#fff" />
              </button>
              <button
                type="button"
                onClick={endCall}
              ...
              >
                <SvgIcon name="phone" size={28} color="#fff" />
              </button>
              {callMode === 'video' && (
                <button
                  type="button"
                ...
                >
                  <SvgIcon name="video" size={22} color="#fff" />
                </button>
              )}
            </div>
          </div>
