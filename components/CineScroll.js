...
function ProfileSheet({onClose,accent,watchlist,setWatchlist,userReviews,loadingData}){
  ...
  const togglePrivacy=async()=>{
    ...
    try{
      await fetch('/api/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({watchlist_public:next})});
      showToast(next?'Watchlist is now public':'Watchlist is now private');
    }catch{setWatchlistPublic(!next);}
  };
  const saveBio=async()=>{
    if(bioInput===bio)return;
    setSavingBio(true);
    try{
      await fetch('/api/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({bio:bioInput})});
      setBio(bioInput);
      showToast('Bio updated');
    }catch{setBioInput(bio);}
    setSavingBio(false);
  };
  const handleCoverUpload=async(e)=>{
    ...
    if(!validTypes.includes(file.type)){showToast('Use JPG, PNG, GIF, or WEBP');return;}
    ...
    try{
      ...
      if(data.cover_url){setCoverUrl(data.cover_url);showToast('Cover photo updated');}
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          {tab==='profile'&&(
            <div style={{padding:'16px'}}>
              ...
              {/* COVER PHOTO UPLOAD */}
              <input ref={coverInputRef} type="file" accept="image/jpeg,image/png,image/gif,image/webp" onChange={handleCoverUpload} style={{display:'none'}}/>
