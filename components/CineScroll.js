...
function loadCanvasImage(url) {
  return new Promise((resolve) => {
    ...
    // Use TMDB proxy to avoid CORS — replace original/w500 with w342
    const safeUrl = url
...
async function generateShareCard(type, data, accent) {
  ...
  // ── Radial glow top ──
  const g1 = ctx.createRadialGradient(W * 0.5, 0, 0, W * 0.5, 0, H * 0.65);
  g1.addColorStop(0, accent + '30');
  g1.addColorStop(0.5, accent + '0a');
  g1.addColorStop(1, 'transparent');
  ctx.fillStyle = g1;
  ctx.fillRect(0, 0, W, H);

  // ── Radial glow bottom ──
  const g2 = ctx.createRadialGradient(W * 0.5, H, 0, W * 0.5, H, H * 0.5);
  g2.addColorStop(0, accent + '20');
  ...
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.roundRect(18, 18, W - 36, H - 36, 26);
  ctx.stroke();

  // ── Top accent bar with gradient ──
  const topBar = ctx.createLinearGradient(0, 0, W, 0);
  topBar.addColorStop(0, 'transparent');
  topBar.addColorStop(0.25, accent + 'cc');
  topBar.addColorStop(0.75, accent + 'cc');
  topBar.addColorStop(1, 'transparent');
  ctx.fillStyle = topBar;
  ctx.beginPath();
  ctx.roundRect(40, 10, W - 80, 3, 2);
  ctx.fill();
  ...
  ctx.lineWidth = 8;
  ctx.beginPath();
  ctx.arc(48, 66, 14, 0, Math.PI * 2);
  ctx.stroke();
  // Wordmark
  ctx.fillStyle = '#ffffff';
  ctx.font = 'italic bold 32px Georgia, serif';
  ctx.textAlign = 'left';
  ctx.fillText('CineScroll', 66, 76);
  ...
  if (type === 'score') {
    ...
    ctx.fillText('CINEPHILE PROFILE', W / 2, 152);
    ...
    // Rank badge
    const rankLabel = data.score < 100 ? '🌱 Newcomer' :
      data.score < 300 ? '🎬 Casual Viewer' :
      data.score < 600 ? '🏆 Dedicated Cinephile' : '👑 Elite Connoisseur';
    ctx.fillStyle = accent + '18';
    ctx.beginPath();
    ctx.roundRect(W / 2 - 130, 240, 260, 38, 19);
    ctx.fill();
    ...
    ctx.textAlign = 'center';
    ctx.fillText(rankLabel, W / 2, 263);
    ...
    if (pct > 0) {
      ...
      arcGrad.addColorStop(0, accent + '80');
      arcGrad.addColorStop(0.5, accent);
      arcGrad.addColorStop(1, accent + 'cc');
      ctx.shadowColor = accent;
      ctx.shadowBlur = 16;
    ...
    // Inner fill
    const innerGrad = ctx.createRadialGradient(cx, cy, 0, cx, cy, r - 20);
    innerGrad.addColorStop(0, accent + '0d');
    ...
    ctx.fillText('CINESCORE', cx, cy + 68);
    ctx.letterSpacing = '0px';

    // ── Stats ──
    const statY = 710;
    const cols = [
      { label: 'WATCHED', value: data.watched, icon: '👁' },
      { label: 'REVIEWS', value: data.reviews, icon: '✍️' },
      { label: 'SAVED',   value: data.saved,   icon: '🔖' },
    ];
    const colW = W / 3;
    cols.forEach((s, i) => {
      ...
      ctx.beginPath();
      ctx.roundRect(colW * i + 24, statY - 46, colW - 48, 100, 18);
      ctx.fill();
      ...
      ctx.textAlign = 'center';
      ctx.fillStyle = '#fff';
      ctx.fillText(s.icon, x, statY - 10);

      ctx.fillStyle = accent;
      ctx.font = 'bold 34px Georgia, serif';
      ctx.fillText(s.value, x, statY + 24);

      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.font = '600 11px sans-serif';
      ctx.letterSpacing = '2px';
      ctx.fillText(s.label, x, statY + 44);
      ctx.letterSpacing = '0px';
    });
    ...
    ctx.textAlign = 'center';
    ctx.fillText('Discover your next favourite film', W / 2, divY + 48);

    // URL pill
    ctx.fillStyle = accent + '15';
    ctx.beginPath();
    ctx.roundRect(W / 2 - 155, divY + 70, 310, 50, 25);
    ctx.fill();
    ...
    ctx.fillText('this-scine.vercel.app', W / 2, divY + 100);
    ...
    ctx.textAlign = 'center';
    ctx.letterSpacing = '4px';
    ctx.fillText('WATCHLIST', W / 2, 148);
    ctx.letterSpacing = '0px';
    ...
    for (const p of pillLabels) {
      ...
      ctx.font = 'bold 14px sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText(p, px + pw / 2, 251);
      px += pw + 16;
    }

    // Divider
    ctx.fillStyle = divGrad;
    ctx.fillRect(36, 282, W - 72, 1);

    // Movie rows with posters
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      if (m.watched) {
        ...
        const cardGrad = ctx.createLinearGradient(36, cardY, W - 36, cardY);
        cardGrad.addColorStop(0, 'rgba(255,255,255,0.06)');
        cardGrad.addColorStop(1, 'rgba(255,255,255,0.03)');
        ctx.fillStyle = cardGrad;
      }
      ctx.beginPath();
      ctx.roundRect(36, cardY, W - 72, CARD_H, 16);
      ctx.fill();

      // Card border
      ctx.strokeStyle = m.watched ? 'rgba(255,255,255,0.04)' : accent + '22';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.roundRect(36, cardY, W - 72, CARD_H, 16);
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.roundRect(posterX, posterY, POSTER_W, POSTER_H, 10);
      ctx.fill();
      ...
      if (m.poster) {
        ...
        if (img) {
          ...
          const dx = posterX + (POSTER_W - dw) / 2;
          const dy = posterY + (POSTER_H - dh) / 2;
          ctx.drawImage(img, dx, dy, dw, dh);
          ctx.restore();
        }
      }

      // Dim overlay for watched
      if (m.watched) {
      ...
      // Poster border
      ctx.strokeStyle = m.watched ? 'rgba(255,255,255,0.08)' : accent + '33';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.roundRect(posterX, posterY, POSTER_W, POSTER_H, 10);
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.roundRect(numX, cardY + CARD_H / 2 - 14, 28, 28, 7);
      ctx.fill();
      ...
      const title = m.title.length > maxTitleChars ? m.title.slice(0, maxTitleChars) + '…' : m.title;
      ...
      ctx.textAlign = 'left';
      ctx.fillText(title, textX, cardY + 36);
      ...
      // RIGHT SIDE — rating pill (always visible, no overlap)
      const ratingX = W - 36 - 76;
      const ratingY = cardY + CARD_H / 2 - 16;
      ctx.fillStyle = accent + '18';
      ctx.beginPath();
      ctx.roundRect(ratingX, ratingY, 68, 32, 16);
      ctx.fill();
      ctx.strokeStyle = accent + '44';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ...
      // WATCHED badge — ABOVE rating pill, no overlap
      if (m.watched) {
        const wY = ratingY - 30;
        ctx.fillStyle = accent + '22';
        ctx.beginPath();
        ctx.roundRect(ratingX, wY, 68, 22, 11);
        ctx.fill();
        ctx.fillStyle = accent;
        ctx.font = 'bold 10px sans-serif';
        ctx.letterSpacing = '1px';
        ctx.textAlign = 'center';
        ctx.fillText('WATCHED', ratingX + 34, wY + 15);
        ctx.letterSpacing = '0px';
      }
    }

    if (data.total > 6) {
      const moreY = startY + 6 * (CARD_H + 8) + 16;
    ...
    ctx.fillStyle = accent + '15';
    ctx.beginPath();
    ctx.roundRect(W / 2 - 155, urlY, 310, 50, 25);
    ctx.fill();
    ctx.strokeStyle = accent + '50';
    ctx.lineWidth = 1;
    ctx.beginPath();
  ...
  botBar.addColorStop(0.25, accent + 'cc');
  botBar.addColorStop(0.75, accent + 'cc');
  botBar.addColorStop(1, 'transparent');
  ctx.fillStyle = botBar;
  ctx.beginPath();
  ctx.roundRect(40, H - 13, W - 80, 3, 2);
  ctx.fill();
...
function Toast({ message, accent }) {
  return (
    <div style={{position:'fixed',top:80,left:'50%',transform:'translateX(-50%)',zIndex:200,background:'rgba(5,5,12,0.96)',backdropFilter:'blur(20px)',border:`1px solid ${accent}44`,borderRadius:24,padding:'12px 24px',display:'flex',alignItems:'center',gap:10,animation:'toastIn 0.3s cubic-bezier(0.22,1,0.36,1)',whiteSpace:'nowrap',boxShadow:`0 8px 32px rgba(0,0,0,0.5),0 0 20px ${accent}18`}}>
...
function MoodScreen({ onClose, onMoodSelect, accent }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:90,background:'rgba(4,4,10,0.98)',backdropFilter:'blur(24px)',display:'flex',flexDirection:'column',animation:'fadeIn 0.25s ease'}}>
      <style>{`@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes moodIn{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}`}</style>
      ...
      <div style={{padding:'20px 20px 0',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
        <div>
          ...
          <div style={{fontSize:13,color:'rgba(255,255,255,0.35)',marginTop:4}}>Pick a mood and we'll find the perfect film</div>
        </div>
        ...
        {FEEL_MOODS.map((mood, i) => {
          ...
          return (
            <button key={mood.label} onClick={() => handleSelect(mood)}
              disabled={loading}
              style={{
                background: isActive ? `${mood.color}20` : 'rgba(255,255,255,0.04)',
                border: `1px solid ${isActive ? mood.color + '66' : 'rgba(255,255,255,0.08)'}`,
                borderRadius: 20,
                padding: '20px 16px',
                cursor: 'pointer',
                display: 'flex',
                ...
                animation: `moodIn 0.3s ease ${i * 0.05}s both`,
                position: 'relative',
                overflow: 'hidden',
              }}>
              {/* Subtle glow overlay on active */}
              {isActive && (
                <div style={{position:'absolute',inset:0,background:`radial-gradient(circle at 30% 50%, ${mood.color}15, transparent 70%)`,pointerEvents:'none'}}/>
              )}
              ...
              {isActive && loading && (
                <div style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',width:16,height:16,border:`2px solid ${mood.color}33`,borderTop:`2px solid ${mood.color}`,borderRadius:'50%',animation:'spin 0.7s linear infinite'}}/>
              )}
            </button>
          );
        })}
...
function ProfileSheet({ onClose, accent, watchlist, setWatchlist, userReviews, loadingData }) {
  ...
  const topGenres = watchlist.flatMap(m => m.genre || [])
  ...
  const handleShareScore = async () => {
    setSharing(true);
    try {
      const dataUrl = await generateShareCard('score', { score:cineScore, name:user?.firstName||user?.username||'Cinephile', watched, reviews, saved }, accent);
      await shareImage(dataUrl, 'My CineScore', `My CineScore is ${cineScore}!`);
  ...
  const handleShareWatchlist = async () => {
    setSharing(true);
    try {
      const dataUrl = await generateShareCard('watchlist', {
        name: user?.firstName||user?.username||'Cinephile',
        total: watchlist.length, watched,
        items: watchlist.map(m => ({ title:m.title, year:m.year, rating:m.rating, watched:m.watched, poster:m.poster, genre:m.genre })),
      }, accent);
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          ...
          {tab==='profile'&&(
            <div style={{padding:'20px'}}>
              ...
              {loadingData&&(
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:16,padding:'10px 14px',background:'rgba(255,255,255,0.03)',borderRadius:12,border:'1px solid rgba(255,255,255,0.06)'}}>
                  <div style={{width:14,height:14,border:`2px solid rgba(255,255,255,0.1)`,borderTop:`2px solid ${accent}`,borderRadius:'50%',animation:'spin 0.7s linear infinite',flexShrink:0}}/>
                  <span style={{fontSize:12,color:'rgba(255,255,255,0.3)'}}>Loading your profile data…</span>
                </div>
              )}
              ...
              <div style={{background:'rgba(255,255,255,0.03)',border:`1px solid ${accent}22`,borderRadius:20,padding:'20px',marginBottom:16,display:'flex',alignItems:'center',gap:20}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:13,color:'rgba(255,255,255,0.6)',lineHeight:1.6,marginBottom:10}}>
                    {cineScore<100?'Just getting started. Watch more films!':cineScore<300?'Casual viewer. Keep exploring.':cineScore<600?'Dedicated cinephile. Impressive.':'Elite film connoisseur. Legendary.'}
                  </div>
            ...
            <div style={{padding:'16px 20px'}}>
              ...
              {loadingData?(
                <div style={{textAlign:'center',padding:40,color:'rgba(255,255,255,0.3)',fontSize:13,display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
                  <div style={{width:24,height:24,border:`2px solid rgba(255,255,255,0.1)`,borderTop:`2px solid ${accent}`,borderRadius:'50%',animation:'spin 0.7s linear infinite'}}/>
                  Loading…
                </div>
              ):watchlist.length===0?(
                  ...
                  {watchlist.map((m,i)=>(
                    <div key={m.movie_id} style={{display:'flex',gap:12,alignItems:'center',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'10px 14px'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        {/* Year · rating · watched badge — all on one line, no overlap */}
                        <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap',marginBottom:6}}>
                          ...
                          {m.watched&&(
                            <span style={{fontSize:9,color:accent,background:`${accent}18`,border:`1px solid ${accent}30`,borderRadius:10,padding:'1px 8px',fontWeight:700,letterSpacing:0.5}}>WATCHED</span>
                          )}
                        </div>
...
function CommentPanel({ movie, onClose, accent, onAuthRequired }) {
  ...
  const post = async () => {
    ...
    if (replyingTo) {
      setComments(p => p.map(c => c.id===replyingTo.id?{...c,replies:[...(c.replies||[]),{id:Date.now(),user:username,avatar,text:input,likes:0,time:'now',liked:false}]}:c));
    } else {
          ...
          <div key={c.id}>
            ...
            {(c.replies||[]).map(r=>(
              <div key={r.id} style={{display:'flex',gap:10,marginTop:10,marginLeft:42}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:2}}><span style={{fontSize:11,fontWeight:600,color:'rgba(255,255,255,0.5)'}}>@{r.user}</span><span style={{fontSize:10,color:'rgba(255,255,255,0.15)'}}>{r.time}</span></div>
...
export default function CineScroll() {
  ...
  // ── Background profile load — fires as soon as Clerk confirms sign-in ──
  useEffect(() => {
    // isLoaded = Clerk has finished initializing
    if (!isLoaded) return;
    if (!isSignedIn) {
      setWatchlist([]);
      setUserReviews([]);
  ...
  // ── Phase 3: handle mood selection ──
  const handleMoodSelect = async (mood) => {
    // Build a discover URL via our API using mood's genre + sort
    const params = new URLSearchParams({
      mood: 'trending',
      genre: mood.genres.split(',')[0], // primary genre
      page: String(Math.floor(Math.random() * 6) + 1),
    });
    try {
    ...
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      <div style={{position:'fixed',top:0,left:0,right:0,zIndex:40,padding:'18px 16px 0',background:'linear-gradient(to bottom,rgba(4,4,10,0.9) 0%,transparent 100%)',display:'flex',justifyContent:'space-between',alignItems:'center',pointerEvents:'none'}}>
        ...
        <div style={{display:'flex',gap:8,pointerEvents:'all',alignItems:'center'}}>
          {/* Mood Discovery button */}
          <button onClick={()=>setShowMood(true)} style={{background:'rgba(0,0,0,0.55)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:12,width:38,height:38,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(12px)',transition:'all 0.2s ease'}} title="Mood Discovery">
            ...
            {(searchQ?searchRes:movies).map((m,i)=>(
              <button key={m.id} onClick={()=>{if(!searchQ){scrollTo(i);}else{setMovies(p=>[m,...p.filter(x=>x.id!==m.id)]);scrollTo(0);}setShowSearch(false);setSearchQ('');}} style={{display:'flex',gap:12,alignItems:'center',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'11px 14px',cursor:'pointer',textAlign:'left',fontFamily:'inherit'}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:'rgba(255,255,255,0.28)',display:'flex',alignItems:'center',gap:5}}>
                    <span>{m.year}</span><span>·</span><SvgIcon name="star" size={10} color={m.accent} filled/><span style={{color:m.accent,fontWeight:600}}>{m.rating}</span>
                  </div>
                </div>
              </button>
            ))}
