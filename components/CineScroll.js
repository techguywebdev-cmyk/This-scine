...
export function InlinePlayer({ movie, onClose, accent, onSave, isSaved, initialTab, highlightCommentId }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:220,background:T.bg,display:'flex',flexDirection:'column',animation:'playerSlideUp 0.4s cubic-bezier(0.22,1,0.36,1)',overflowY:'auto',WebkitOverflowScrolling:'touch'}}>
...
function ProfileSheet({onClose,accent,watchlist,setWatchlist,userReviews,loadingData,onDiscover}){
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          ...
          {tab==='watchlist'&&(
            <div>
              ...
              :filteredWatchlist.length===0?(<div style={{textAlign:'center',padding:'24px 20px',display:'flex',flexDirection:'column',alignItems:'center',gap:12}}><SvgIcon name="bookmark" size={26} color={T.hairlineStrong}/><div style={{fontSize:13.5,color:T.text3}}>{watchlistSearch?'No matches':'Your watchlist is empty'}</div>{!watchlistSearch&&<button onClick={()=>{onClose&&onClose();onDiscover&&onDiscover();}} style={{marginTop:4,background:accent,border:'none',borderRadius:20,padding:'10px 18px',cursor:'pointer',fontSize:12,fontWeight:700,color:'#07070F',fontFamily:'inherit'}}>Discover movies</button>}</div>)
              :(
                <div style={{display:'flex',flexDirection:'column',padding:'0 18px 12px'}}>
                  ...
                  <button
                    onClick={()=>{ onClose&&onClose(); onDiscover&&onDiscover(); }}
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      {(activeGenre||activeProvider||(activeMood&&activeMood!=='Trending'))&&!showFilter&&(
        <div style={{position:'fixed',top:108,left:16,zIndex:38,display:'flex',gap:6,flexWrap:'wrap',maxWidth:'70%'}}>
      ...
      {showProfile&&<ProfileSheet onClose={()=>setShowProfile(false)} accent={accent} watchlist={watchlist} setWatchlist={setWatchlist} userReviews={userReviews} loadingData={loadingProfileData} onDiscover={()=>{setShowProfile(false);setShowFilter(true);}}/>}
