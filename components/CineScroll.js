...
function CoverCropModal({file,onCancel,onSave,accent,aspect=2.2,title='Adjust Cover Photo',outputWidth=1200,roundPreview=false}){
  ...
  const handleSave=()=>{
    ...
    console.log('[CROP DEBUG]',{natural,containerSize,baseScale,scale,effectiveScale,displayW,displayH,offset,srcX,srcY,srcW,srcH,srcAspect:srcW/srcH,OUT_W,OUT_H,outAspect:OUT_W/OUT_H});
...
function ProfileSheet({onClose,accent,watchlist,setWatchlist,userReviews,loadingData}){
  ...
  const coverDisplayRef=useRef(null);
  useEffect(()=>{
    if(coverDisplayRef.current){
      const r=coverDisplayRef.current.getBoundingClientRect();
      console.log('[COVER DISPLAY DEBUG]',{width:r.width,height:r.height,aspect:r.width/r.height,coverUrl});
    }
  },[coverUrl]);
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          {tab==='profile'&&(
            <div style={{padding:'16px'}}>
              ...
              <button ref={coverDisplayRef} onClick={()=>coverInputRef.current?.click()} disabled={uploadingCover} style={{position:'relative',width:'100%',aspectRatio:'2.2',borderRadius:18,overflow:'hidden',border:`1px solid ${T.hairline}`,background:coverUrl?`url(${coverUrl})`:`linear-gradient(150deg,${accent}1f,${T.surface})`,backgroundSize:'cover',backgroundPosition:'center',cursor:'pointer',padding:0,display:'block'}}>
