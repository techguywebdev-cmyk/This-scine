...
function StreamingBadges({ movieId, mediaType, title }) {
  ...
  const providerDeepLink = (p) => {
    const q = encodeURIComponent(title || '');
    const name = (p.provider_name || '').toLowerCase();
    // Prefer TMDB's official watch page (accurate for this title + region)
    if (watchLink) return watchLink;
    // Search fallbacks if TMDB link missing
  ...
  const openProvider = (p, e) => {
    e?.stopPropagation?.();
    e?.preventDefault?.();
    const url = providerDeepLink(p);
    if (url) window.open(url, '_blank', 'noopener,noreferrer');
  };

  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
      <span style={{ fontSize: 9, letterSpacing: 1.5, color: 'rgba(255,255,255,0.35)', fontWeight: 600, textTransform: 'uppercase' }}>
        Stream on
      </span>
    ...
    <div style={{position:'fixed',inset:0,zIndex:260,background:T.bg,display:'flex',flexDirection:'column',animation:'playerSlideUp 0.4s cubic-bezier(0.22,1,0.36,1)',overflowY:'auto',WebkitOverflowScrolling:'touch'}}>
      ...
      {activeTab==='about'&&(
        <div style={{padding:'18px',animation:'fadeIn 0.2s ease'}}>
          ...
          <StreamingBadges movieId={movie?.id} mediaType={movie?.mediaType} title={movie?.title}/>
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          ...
          {tab==='watchlist'&&(
            <div>
              ...
              :filteredWatchlist.length===0?(<div style={{textAlign:'center',padding:'24px 20px',display:'flex',flexDirection:'column',alignItems:'center',gap:12}}><SvgIcon name="bookmark" size={26} color={T.hairlineStrong}/><div style={{fontSize:13.5,color:T.text3}}>{watchlistSearch?'No matches':'Your watchlist is empty'}</div>{!watchlistSearch&&<button onClick={()=>{if(onDiscover){onDiscover();}else if(onClose){onClose();}}} style={{marginTop:4,background:accent,border:'none',borderRadius:20,padding:'10px 18px',cursor:'pointer',fontSize:12,fontWeight:700,color:'#07070F',fontFamily:'inherit'}}>Discover movies</button>}</div>)
              :(
...
function FilterSheet({ show, onClose, activeGenre, activeMood, onGenre, onMood, accent, onSearchSelect, activeProvider, onProvider }) {
  ...
  // Rotate popular searches whenever the sheet opens
  useEffect(() => {
    if (!show) return;
  ...
  const handlePlatform = (p) => {
    ...
    if (on) {
      ...
      // keep current mood but refresh feed for this platform
...
function MovieCard({movie,isActive,index,onFindSimilar,onAuthRequired,onSave,isSaved,onTrailer}){
  ...
  return(
    <div style={{position:'relative',width:'100%',height:'100%',overflow:'hidden',background:'#04040A',userSelect:'none',WebkitUserSelect:'none'}} onMouseDown={onPressStart} onMouseUp={onPressEnd} onMouseLeave={onPressEnd} onTouchStart={onPressStart} onTouchEnd={onPressEnd} onTouchCancel={onPressEnd}>
      ...
      <div style={{position:'absolute',top:118,left:0,right:0,zIndex:10,padding:'0 16px',display:'flex',justifyContent:'space-between',alignItems:'center',opacity:isActive?1:0.5,transition:'opacity 0.4s ease'}}>
      ...
      <div style={{position:'absolute',bottom:0,left:0,right:68,padding:'0 20px 36px',zIndex:10,opacity:isActive?1:0.4,transform:isActive?'translateY(0)':'translateY(18px)',transition:'all 0.5s ease'}}>
        ...
        {isActive&&<StreamingBadges movieId={movie.id} mediaType={movie.mediaType} title={movie.title}/>}
  ...
  const postComment = async () => {
    ...
    try {
      const res = await fetch('/api/reviews', {
        ...
        body: JSON.stringify({ listId, text: commentInput, rating: 0 }),
      });
      const data = await res.json();
      if (data.comment) {
        setComments((p) => [data.comment, ...p]);
              ...
              comments.map((c, i) => (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 13, color: T.text2, lineHeight: 1.5, margin: 0 }}>{c.text}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div style={{padding:'0 20px'}}>
            ...
            {activeTab==='discussion'&&(
              <div style={{display:'flex',flexDirection:'column',minHeight:280}}>
                ...
                {isSignedIn?(
                  <div style={{display:'flex',gap:9,marginTop:16,paddingTop:14,borderTop:`1px solid ${T.hairline}`}}>
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      {(activeGenre||activeProvider||(activeMood&&activeMood!=='Trending'))&&!showFilter&&(
        <div style={{position:'fixed',top:56,left:16,zIndex:38,display:'flex',gap:6,flexWrap:'wrap',maxWidth:'70%'}}>
