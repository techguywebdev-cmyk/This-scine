...
// INLINE PLAYER
function InlinePlayer({ movie, onClose, accent, onSave, isSaved, initialTab, highlightCommentId }) {
      ...
      {activeTab==='comments'&&(
        <div style={{display:'flex',flexDirection:'column',animation:'fadeIn 0.2s ease'}}>
          ...
          <div style={{padding:'12px 16px',display:'flex',flexDirection:'column',gap:14}}>
            {loadingComments?(
              <div style={{display:'flex',justifyContent:'center',padding:24}}><div style={{width:22,height:22,border:`2px solid rgba(255,255,255,0.1)`,borderTop:`2px solid ${accent}`,borderRadius:'50%',animation:'spin 0.8s linear infinite'}}/></div>
            ):comments.length===0?(
              <div style={{textAlign:'center',padding:'24px 0',fontSize:13,color:'rgba(255,255,255,0.25)'}}>No comments yet. Be the first!</div>
            ):comments.map(c=>(
              <div key={c.id} id={`comment-${c.id}`} style={{borderRadius:10,padding:'2px 4px',margin:'-2px -4px'}}>
                <div style={{display:'flex',gap:10}}>
                  <button onClick={()=>setViewingProfile(c.user_id)} style={{width:34,height:34,borderRadius:'50%',background:`${accent}20`,border:`1px solid ${accent}40`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:accent,flexShrink:0,overflow:'hidden',padding:0,cursor:'pointer'}}>
                    {c.avatar_url?<img src={c.avatar_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:(c.username||'U')[0].toUpperCase()}
                  </button>
                  <div style={{flex:1}}>
                    <div style={{display:'flex',alignItems:'center',gap:7,marginBottom:4}}>
                      <button onClick={()=>setViewingProfile(c.user_id)} style={{background:'none',border:'none',cursor:'pointer',padding:0,fontFamily:'inherit'}}><span style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,0.8)'}}>@{c.username}</span></button>
                      ...
                      <span style={{fontSize:10,color:'rgba(255,255,255,0.18)',marginLeft:'auto'}}>{timeAgo(c.created_at)}</span>
                    </div>
                    ...
                    <div style={{display:'flex',gap:14,alignItems:'center'}}>
                      <button onClick={()=>toggleLike(c.id)} style={{background:'none',border:'none',cursor:'pointer',padding:0,display:'flex',alignItems:'center',gap:4}}>
                        <SvgIcon name="heart" size={13} color={likedLocal[c.id]?'#FF6B8A':'rgba(255,255,255,0.2)'} filled={!!likedLocal[c.id]}/>
                      </button>
          ...
          <div style={{padding:'12px 16px 32px',borderTop:'1px solid rgba(255,255,255,0.06)',marginTop:8}}>
            {replyingTo&&(
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8,fontSize:11,color:'rgba(255,255,255,0.4)'}}>
                <span>Replying to <span style={{color:accent}}>@{replyingTo.username}</span></span>
                <button onClick={()=>{setReplyingTo(null);setCommentInput('');}} style={{background:'none',border:'none',cursor:'pointer',color:'rgba(255,255,255,0.3)',fontSize:14,padding:0}}>x</button>
              </div>
            )}
            {!replyingTo&&(
            ...
            {isSignedIn?(
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input ref={commentInputRef} value={commentInput} onChange={e=>setCommentInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&postComment()} placeholder={replyingTo?`Reply to @${replyingTo.username}...`:'Comment on this trailer...'} style={{flex:1,background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:22,padding:'12px 16px',color:'#fff',fontSize:14,outline:'none',fontFamily:'inherit'}}/>
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:100,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(16px)',display:'flex',alignItems:'flex-end',animation:'fadeIn 0.2s ease'}}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',height:'92%',background:'rgba(8,8,16,0.99)',borderRadius:'24px 24px 0 0',border:'1px solid rgba(255,255,255,0.08)',borderBottom:'none',display:'flex',flexDirection:'column',animation:'sheetUp 0.35s cubic-bezier(0.22,1,0.36,1)'}}>
        ...
        <div style={{padding:'14px 16px 0',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          ...
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <button onClick={()=>setShowOwnPreview(true)} title="Preview public profile" style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,width:32,height:32,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="eye" size={14} color="rgba(255,255,255,0.4)"/></button>
            <button onClick={()=>{bioRef.current?.scrollIntoView({behavior:'smooth',block:'center'});setTimeout(()=>bioRef.current?.focus(),300);}} title="Edit Profile" style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,width:32,height:32,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="edit" size={14} color="rgba(255,255,255,0.4)"/></button>
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          {tab==='profile'&&(
            <div style={{padding:'16px'}}>
              ...
              <button onClick={()=>coverInputRef.current?.click()} disabled={uploadingCover} style={{position:'relative',width:'100%',height:100,borderRadius:16,overflow:'hidden',border:'1px solid rgba(255,255,255,0.08)',background:coverUrl?`url(${coverUrl})`:`linear-gradient(135deg,${accent}25,rgba(255,255,255,0.03))`,backgroundSize:'cover',backgroundPosition:'center',cursor:'pointer',padding:0,marginBottom:14,display:'flex',alignItems:'center',justifyContent:'center'}}>
                <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.35)'}}/>
                {uploadingCover?(
                  <div style={{position:'relative',zIndex:1,width:22,height:22,border:'2px solid rgba(255,255,255,0.2)',borderTop:'2px solid #fff',borderRadius:'50%',animation:'spin 0.7s linear infinite'}}/>
                ):(
                  <div style={{position:'relative',zIndex:1,display:'flex',alignItems:'center',gap:8,background:'rgba(0,0,0,0.5)',borderRadius:20,padding:'7px 14px'}}>
                    <SvgIcon name="plus" size={13} color="#fff"/>
                    <span style={{fontSize:12,fontWeight:600,color:'#fff'}}>{coverUrl?'Change cover photo':'Add cover photo'}</span>
                  </div>
                )}
              </button>
              ...
              <div style={{background:'linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))',border:'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:'16px',marginBottom:14,display:'flex',alignItems:'center',gap:14}}>
                ...
                <button onClick={()=>avatarInputRef.current?.click()} disabled={uploadingAvatar} style={{position:'relative',width:68,height:68,borderRadius:'50%',background:`linear-gradient(135deg,${accent}44,${accent}22)`,border:`2px solid ${accent}66`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,overflow:'hidden',padding:0,cursor:'pointer'}}>
                  ...
                  <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center'}}>
                    {uploadingAvatar?<div style={{width:16,height:16,border:'2px solid rgba(255,255,255,0.3)',borderTop:'2px solid #fff',borderRadius:'50%',animation:'spin 0.7s linear infinite'}}/>:<SvgIcon name="edit" size={15} color="#fff"/>}
                  </div>
                </button>
                <div style={{flex:1}}>
                  <div style={{fontSize:18,fontWeight:700,color:'#fff',fontFamily:"'Playfair Display',serif",fontStyle:'italic',marginBottom:2}}>{nickname||user?.firstName||user?.username||'Cinephile'}</div>
              ...
              <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'14px',marginBottom:14}}>
                <div style={{fontSize:9,letterSpacing:2,color:'rgba(255,255,255,0.3)',fontWeight:700,marginBottom:8,textTransform:'uppercase'}}>Display Name</div>
                <input value={nicknameInput} onChange={e=>setNicknameInput(e.target.value)} maxLength={40} placeholder={user?.firstName||user?.username||'Your name'} style={{width:'100%',boxSizing:'border-box',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'10px 12px',color:'#fff',fontSize:14,outline:'none',fontFamily:'inherit'}}/>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:8}}>
                  <span style={{fontSize:10,color:'rgba(255,255,255,0.2)'}}>Shown instead of your name · @{user?.username||'handle'} stays the same</span>
                  {nicknameInput!==nickname&&(
                    <button onClick={saveNickname} disabled={savingNickname} style={{background:savingNickname?'rgba(255,255,255,0.08)':accent,border:'none',borderRadius:14,padding:'5px 16px',cursor:savingNickname?'default':'pointer',fontSize:11,fontWeight:700,color:savingNickname?'rgba(255,255,255,0.4)':'#07070F',fontFamily:'inherit',display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
              ...
              {/* BIO */}
              <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'14px',marginBottom:14}}>
                <div style={{fontSize:9,letterSpacing:2,color:'rgba(255,255,255,0.3)',fontWeight:700,marginBottom:8,textTransform:'uppercase'}}>Bio</div>
                <textarea ref={bioRef} value={bioInput} onChange={e=>setBioInput(e.target.value)} maxLength={160} placeholder="Tell people about your taste in film..." rows={2} style={{width:'100%',boxSizing:'border-box',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:10,padding:'10px 12px',color:'#fff',fontSize:13,outline:'none',fontFamily:'inherit',resize:'none',lineHeight:1.5}}/>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:8}}>
                  <span style={{fontSize:10,color:'rgba(255,255,255,0.2)'}}>{bioInput.length}/160</span>
                  {bioInput!==bio&&(
                    <button onClick={saveBio} disabled={savingBio} style={{background:savingBio?'rgba(255,255,255,0.08)':accent,border:'none',borderRadius:14,padding:'5px 16px',cursor:savingBio?'default':'pointer',fontSize:11,fontWeight:700,color:savingBio?'rgba(255,255,255,0.4)':'#07070F',fontFamily:'inherit',display:'flex',alignItems:'center',gap:6}}>
...
// USER PROFILE SHEET
function UserProfileSheet({userId,onClose,accent,onWatchTrailer,onAddToWatchlist}){
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxHeight:'90vh',background:'rgba(8,8,16,0.99)',borderRadius:'24px 24px 0 0',border:'1px solid rgba(255,255,255,0.08)',borderBottom:'none',display:'flex',flexDirection:'column',animation:'sheetUp 0.32s cubic-bezier(0.22,1,0.36,1)',overflow:'hidden'}}>
        ...
        {loading?(
          ...
          <div className="profile-scroll" style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
            {/* COVER PHOTO */}
            <div style={{position:'relative',height:178,flexShrink:0,background:coverImg?`url(${coverImg})`:`linear-gradient(135deg,${accentColor}30,#0a0a14)`,backgroundSize:'cover',backgroundPosition:'center 18%',transition:'background-image 0.4s ease'}}>
              <div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,rgba(8,8,16,0.05) 0%,rgba(8,8,16,0.45) 55%,rgba(8,8,16,0.98) 100%)'}}/>
              ...
              <button onClick={onClose} style={{position:'absolute',top:14,right:14,background:'rgba(0,0,0,0.4)',backdropFilter:'blur(8px)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'50%',width:30,height:30,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',zIndex:2}}><SvgIcon name="close" size={13} color="rgba(255,255,255,0.8)"/></button>
              {/* Avatar anchored to the bottom of the cover, overlapping by half its height */}
              <div style={{position:'absolute',left:20,bottom:-38,width:76,height:76,borderRadius:'50%',background:`${accentColor}25`,border:'3px solid #08080F',display:'flex',alignItems:'center',justifyContent:'center',fontSize:28,fontWeight:700,color:accentColor,overflow:'hidden',zIndex:2}}>
            ...
            <div style={{padding:'0 20px 32px',marginTop:48}}>
              {/* HEADER: name + follow */}
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:18}}>
                {[
                  {label:'Followers',value:profile.followers,icon:'people'},
                  {label:'Following',value:profile.following,icon:'userPlus'},
                  {label:'Watchlist',value:profile.watchlistCount,icon:'bookmark'},
                ].map(s=>(
                  <div key={s.label} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'12px 8px',display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
              ...
              {tab==='watchlist'&&(
                profile.watchlist===null?(
                  <div style={{textAlign:'center',padding:'24px 16px',display:'flex',flexDirection:'column',alignItems:'center',gap:8,background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:14}}>
                    <SvgIcon name="bookmark" size={22} color="rgba(255,255,255,0.12)"/>
                    <div style={{fontSize:12,color:'rgba(255,255,255,0.3)'}}>This watchlist is private</div>
                  </div>
                ):profile.watchlist.length===0?(
                  <div style={{textAlign:'center',padding:'24px 16px',fontSize:12,color:'rgba(255,255,255,0.25)'}}>No titles yet</div>
                ):(
              ...
              {tab==='reviews'&&(
                loadingReviews?(
                  ...
                  <div style={{textAlign:'center',padding:'24px 16px',fontSize:12,color:'rgba(255,255,255,0.25)'}}>No reviews yet</div>
                ):(
                  <div style={{display:'flex',flexDirection:'column',gap:10}}>
                    {reviews.map(r=>(
                      <div key={r.id} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'14px'}}>
                        ...
                        {r.rating>0&&<div style={{display:'flex',gap:2,marginBottom:6}}>{[1,2,3,4,5].map(s=><SvgIcon key={s} name="star" size={11} color={s<=r.rating?accentColor:'rgba(255,255,255,0.12)'} filled={s<=r.rating}/>)}</div>}
                        <p style={{fontSize:13,color:'rgba(255,255,255,0.6)',lineHeight:1.55,margin:0}}>{r.text}</p>
                      </div>
                    ))}
                  </div>
              ...
              {tab==='lists'&&(
                <div style={{textAlign:'center',padding:'32px 16px',display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
                  <SvgIcon name="list" size={26} color="rgba(255,255,255,0.12)"/>
                  <div style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,0.35)'}}>No lists yet</div>
                  <div style={{fontSize:11,color:'rgba(255,255,255,0.2)'}}>Custom lists are coming soon</div>
                </div>
              )}

              {/* FIND FRIENDS CTA (other profiles only) */}
              {!profile.isSelf&&(
                <div style={{marginTop:20,display:'flex',alignItems:'center',gap:12,background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'14px'}}>
                  <div style={{width:36,height:36,borderRadius:10,background:`${accentColor}15`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><SvgIcon name="people" size={16} color={accentColor}/></div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:13,fontWeight:700,color:accentColor}}>Find Friends</div>
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.3)',marginTop:1}}>Connect and see what your friends are watching</div>
                  </div>
                  <span style={{display:'flex',transform:'rotate(-90deg)'}}><SvgIcon name="chevron" size={14} color="rgba(255,255,255,0.2)"/></span>
                </div>
              )}
            </div>
          </div>
            ...
            <div style={{position:'relative',zIndex:1}}>
              ...
              <div style={{position:'relative'}}>
                ...
                <input
                  ...
                  style={{width:'100%',boxSizing:'border-box',background:'rgba(255,255,255,0.07)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:28,padding:'13px 42px 13px 42px',color:'#fff',fontSize:15,outline:'none',fontFamily:'inherit'}}
                />
                {searchQ&&(
                  <button onClick={()=>setSearchQ('')} style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'rgba(255,255,255,0.1)',border:'none',borderRadius:'50%',width:24,height:24,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
                    <SvgIcon name="close" size={11} color="rgba(255,255,255,0.6)"/>
                  </button>
                )}
              </div>
            </div>
...
function NotificationsPanel({onClose,accent,notifications,loading,onMarkRead,onFollowBack}){
  ...
  return(
    <>
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:105,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(6px)',animation:'fadeIn 0.2s ease'}}/>
    <div style={{position:'fixed',top:0,left:0,right:0,zIndex:106,background:'rgba(8,8,16,0.99)',borderRadius:'0 0 24px 24px',border:'1px solid rgba(255,255,255,0.08)',borderTop:'none',maxHeight:'70vh',display:'flex',flexDirection:'column',animation:'notifDrop 0.32s cubic-bezier(0.22,1,0.36,1)',paddingTop:'env(safe-area-inset-top,0px)'}}>
      ...
      <div style={{padding:'18px 16px 14px',display:'flex',justifyContent:'space-between',alignItems:'center',borderBottom:'1px solid rgba(255,255,255,0.06)',flexShrink:0}}>
        <span style={{fontFamily:"'Playfair Display',serif",fontSize:19,fontWeight:900,fontStyle:'italic',color:'#fff'}}>Notifications</span>
        <button onClick={onClose} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:'50%',width:30,height:30,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="close" size={13} color="rgba(255,255,255,0.5)"/></button>
      </div>
      <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
        {loading?(
          ...
          <div style={{textAlign:'center',padding:'40px 24px',display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
            <SvgIcon name="inbox" size={32} color="rgba(255,255,255,0.1)"/>
            <div style={{fontSize:14,color:'rgba(255,255,255,0.35)',fontWeight:600}}>No notifications yet</div>
            <div style={{fontSize:12,color:'rgba(255,255,255,0.2)'}}>New followers and activity will show up here</div>
          </div>
        ):(
          <div style={{padding:'4px 16px 16px'}}>
            {notifications.map((n,i)=>{
              ...
              return(
                <div key={n.id||i} onClick={()=>!n.read&&onMarkRead(n.id)} style={{display:'flex',gap:12,alignItems:'center',padding:'12px 0',borderBottom:i<notifications.length-1?'1px solid rgba(255,255,255,0.04)':'none',cursor:n.read?'default':'pointer',opacity:n.read?0.5:1,transition:'opacity 0.2s ease'}}>
                  <div style={{width:40,height:40,borderRadius:'50%',background:`${ni.color}18`,border:`1px solid ${ni.color}40`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,position:'relative',overflow:'hidden'}}>
                    {n.avatar_url?<img src={n.avatar_url} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<SvgIcon name={ni.icon} size={17} color={ni.color}/>}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.25)',marginTop:2}}>{timeAgo(n.created_at)} ago</div>
                  </div>
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 90, background: '#08080F', display: 'flex', flexDirection: 'column', animation: 'playerSlideUp 0.4s cubic-bezier(0.22,1,0.36,1)' }}>
      ...
      <div style={{ padding: '52px 16px 0', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', margin: 0 }}>{stats.following} following · {stats.followers} followers</p>
        </div>
        <button onClick={() => setTab('discover')} style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, width: 36, height: 36, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <SvgIcon name="userPlus" size={16} color="rgba(255,255,255,0.7)" />
        </button>
        <button onClick={() => setShowNotifs(true)} style={{ position: 'relative', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, width: 36, height: 36, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <SvgIcon name="bell" size={16} color="rgba(255,255,255,0.7)" />
          {unreadCount > 0 && <div style={{ position: 'absolute', top: -2, right: -2, width: 10, height: 10, borderRadius: '50%', background: accent, border: '2px solid #08080F' }} />}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', padding: '14px 16px 0', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
        {[['feed', 'flame', 'Feed'], ['friends', 'people', 'Friends'], ['discover', 'search', 'Discover']].map(([t, icon, label]) => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, background: 'none', border: 'none', cursor: 'pointer', padding: '8px 0 12px', fontFamily: 'inherit', fontSize: 13, fontWeight: tab === t ? 700 : 400, color: tab === t ? accent : 'rgba(255,255,255,0.35)', borderBottom: `2px solid ${tab === t ? accent : 'transparent'}`, transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            <SvgIcon name={icon} size={14} color={tab === t ? accent : 'rgba(255,255,255,0.35)'} filled={t === 'feed' && tab === t} />
            {label}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}>
          ...
          <div style={{ padding: '16px' }}>
            {!isSignedIn ? (
              <div style={{ textAlign: 'center', padding: '48px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
                <SvgIcon name="people" size={42} color="rgba(255,255,255,0.12)" />
              ...
              <div>
                {/* FOLLOW BANNER */}
                {stats.following === 0 && (
                  <div style={{ position: 'relative', background: 'rgba(255,255,255,0.03)', border: `1px solid ${accent}25`, borderRadius: 20, padding: '22px 18px', marginBottom: 20, overflow: 'hidden' }}>
                    <div style={{ position: 'absolute', right: -10, top: -10, width: 120, height: 120, opacity: 0.9 }}>
                      <svg viewBox="0 0 24 24" width="100%" height="100%" fill="none">
                        <circle cx="14" cy="7" r="4.2" fill={`${accent}35`} />
                        <circle cx="7" cy="10" r="3.2" fill={`${accent}25`} />
                        <circle cx="18" cy="12" r="2.6" fill={`${accent}20`} />
                        <path d="M14 11.2c-3.5 0-6.5 1.8-6.5 4.6V18h13v-2.2c0-2.8-3-4.6-6.5-4.6z" fill={`${accent}40`} />
                        <path d="M7 13.2c-2.6 0-4.8 1.4-4.8 3.4V18h5.5" fill={`${accent}28`} />
                      </svg>
                    </div>
                    <div style={{ position: 'relative', zIndex: 1, maxWidth: '70%' }}>
                      <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 19, fontWeight: 800, fontStyle: 'italic', color: '#fff', lineHeight: 1.3, marginBottom: 8 }}>
                        Follow friends to see <span style={{ color: accent }}>what</span> they're watching
                      </div>
                      <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 14 }}>Search for users in the Discover tab</div>
                ...
                {/* ACTIVITY HEADER */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <div style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(255,255,255,0.25)', fontWeight: 700, textTransform: 'uppercase' }}>Activity Feed</div>
                  <div style={{ position: 'relative' }}>
                    <button onClick={() => setShowFilterMenu(p => !p)} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 16, padding: '5px 10px', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11, color: 'rgba(255,255,255,0.6)', fontWeight: 600 }}>
                      {ACTIVITY_FILTERS.find(f => f.id === activityFilter)?.label}
                      <SvgIcon name="chevron" size={11} color="rgba(255,255,255,0.4)" />
                    </button>
                    {showFilterMenu && (
                      <>
                        <div onClick={() => setShowFilterMenu(false)} style={{ position: 'fixed', inset: 0, zIndex: 10 }} />
                        <div style={{ position: 'absolute', top: '110%', right: 0, zIndex: 11, background: 'rgba(14,14,22,0.99)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: 4, minWidth: 130, animation: 'menuIn 0.15s ease', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                          {ACTIVITY_FILTERS.map(f => (
                            <button key={f.id} onClick={() => { setActivityFilter(f.id); setShowFilterMenu(false); }} style={{ display: 'block', width: '100%', textAlign: 'left', background: activityFilter === f.id ? `${accent}15` : 'none', border: 'none', borderRadius: 8, padding: '8px 10px', cursor: 'pointer', fontFamily: 'inherit', fontSize: 12, color: activityFilter === f.id ? accent : 'rgba(255,255,255,0.6)', fontWeight: activityFilter === f.id ? 700 : 400 }}>{f.label}</button>
                          ))}
                        </div>
                      </>
                    )}
                ...
                {filteredFeed.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '32px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                    <SvgIcon name="flame" size={28} color="rgba(255,255,255,0.1)" />
                    <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)' }}>{stats.following === 0 ? 'No activity yet' : 'Nothing matches this filter'}</div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                    {filteredFeed.map((item, i) => {
                      const act = activityIcon(item.type);
                      return (
                        <div key={item.id || i} style={{ display: 'flex', gap: 12, padding: '14px 0', borderBottom: i < filteredFeed.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', animation: 'fadeIn 0.3s ease' }}>
                          ...
                          <button onClick={() => setViewingProfile(item.user_id)} style={{ position: 'relative', flexShrink: 0, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                            <div style={{ width: 42, height: 42, borderRadius: '50%', background: `${act.color}18`, border: `1px solid ${act.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                              {item.avatar_url ? <img src={item.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: 15, fontWeight: 800, color: act.color }}>{(item.username || 'U')[0].toUpperCase()}</span>}
                            </div>
                            <div style={{ position: 'absolute', bottom: -2, right: -2, width: 18, height: 18, borderRadius: '50%', background: act.color, border: '2px solid #08080F', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                              <SvgIcon name={act.icon} size={9} color="#08080F" filled={act.icon === 'bookmark' || act.icon === 'eye'} />
                            </div>
                          </button>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6, flexWrap: 'wrap' }}>
                              <button onClick={() => setViewingProfile(item.user_id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontFamily: 'inherit' }}><span style={{ fontSize: 13, fontWeight: 700, color: 'rgba(255,255,255,0.85)' }}>@{item.username || 'user'}</span></button>
                            ...
                            {item.movie_title && (
                              <button onClick={() => onWatchTrailer({ id: item.movie_id, title: item.movie_title, poster: item.movie_poster, year: item.movie_year, rating: item.movie_rating, accent: item.movie_accent || accent, mediaType: 'movie', ...(item.type === 'reviewed' && item.review_id ? { initialTab: 'comments', highlightCommentId: item.review_id } : {}) })}
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  {item.review_text && <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', margin: '8px 0 0', lineHeight: 1.5, background: 'rgba(0,0,0,0.25)', borderRadius: 8, padding: '8px 10px', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{item.review_text}</p>}
                                </div>
                                {item.type === 'saved' ? (
                                  <div style={{ width: 30, height: 30, borderRadius: '50%', border: `1px solid ${accent}44`, background: `${accent}12`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <SvgIcon name="plus" size={14} color={accent} />
                                  </div>
                                ) : (
                                  <SvgIcon name="play" size={13} color="rgba(255,255,255,0.3)" filled />
                                )}
                ...
                <button onClick={() => setTab('discover')} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '14px', cursor: 'pointer', fontFamily: 'inherit', marginTop: 16 }}>
              ...
              <div>
                ...
                <div style={{ position: 'relative', marginBottom: 14 }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '12px 16px 12px 40px', color: '#fff', fontSize: 14, outline: 'none', fontFamily: 'inherit' }}
                  />
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 20 }}>
                  {[
                    ...
                    <div key={s.label} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '14px 8px', textAlign: 'center', position: 'relative' }}>
                      {s.badge && <div style={{ position: 'absolute', top: 8, right: 8, width: 8, height: 8, borderRadius: '50%', background: accent }} />}
                      <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 8px' }}>
                        <SvgIcon name={s.icon} size={14} color="rgba(255,255,255,0.4)" />
                      </div>
                      <div style={{ fontSize: 18, fontWeight: 800, color: '#fff', fontFamily: "'Playfair Display',serif" }}>{s.value}</div>
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', fontWeight: 600, marginTop: 2 }}>{s.label}</div>
                    </div>
                  ))}
                </div>
                ...
                {loadingFriends ? (
                  ...
                  <div>
                    {/* EMPTY STATE */}
                    <div style={{ textAlign: 'center', padding: '24px 20px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <div style={{ position: 'relative', width: 90, height: 70, marginBottom: 8 }}>
                        <svg viewBox="0 0 24 24" width="100%" height="100%" fill="none">
                          <circle cx="12" cy="6" r="3.4" fill="rgba(255,255,255,0.12)" />
                          <circle cx="5" cy="9" r="2.6" fill="rgba(255,255,255,0.08)" />
                          <circle cx="19" cy="9" r="2.6" fill="rgba(255,255,255,0.08)" />
                          <path d="M12 10.4c-3.4 0-6.2 1.7-6.2 4.4v2.2h12.4v-2.2c0-2.7-2.8-4.4-6.2-4.4z" fill="rgba(255,255,255,0.1)" />
                          <path d="M5 12.6c-2.2 0-4 1.2-4 3v1.6h4.5" fill="rgba(255,255,255,0.06)" />
                          <path d="M19 12.6c2.2 0 4 1.2 4 3v1.6h-4.5" fill="rgba(255,255,255,0.06)" />
                        </svg>
                        <div style={{ position: 'absolute', bottom: 0, right: 8, width: 26, height: 26, borderRadius: '50%', background: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid #08080F' }}>
                          <SvgIcon name="plus" size={13} color="#07070F" />
                        </div>
                      </div>
                      <div style={{ fontSize: 16, fontWeight: 800, color: '#fff', fontFamily: "'Playfair Display',serif" }}>{friendsSearchQ ? 'No matches' : 'No friends yet'}</div>
                      {!friendsSearchQ && (
                        <>
                          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', lineHeight: 1.6, maxWidth: 260 }}>Find and follow friends to see their activity in your feed.</div>
                    ...
                    {loadingSuggested ? (
                      ...
                      <div style={{ textAlign: 'center', padding: '16px 0', fontSize: 12, color: 'rgba(255,255,255,0.25)' }}>No suggestions right now</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {suggested.map((u, i) => (
                          <div key={u.user_id || i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '12px 14px' }}>
                            <button onClick={() => setViewingProfile(u.user_id)} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left', fontFamily: 'inherit' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                                  <span style={{ fontSize: 14, fontWeight: 700, color: '#fff' }}>{u.display_name || u.username}</span>
                                  {u.verified && <SvgIcon name="badgeCheck" size={13} color="#4DA8FF" filled />}
                                </div>
                                <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>@{u.username}</div>
                                {u.mutualCount > 0 && <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', marginTop: 2 }}>{u.mutualCount} mutual friend{u.mutualCount === 1 ? '' : 's'}</div>}
                              </div>
                            </button>
                            <button onClick={() => handleFollow(u)}
                              style={{ background: u.isFollowing ? 'rgba(255,255,255,0.06)' : 'none', border: `1px solid ${u.isFollowing ? 'rgba(255,255,255,0.1)' : accent}`, borderRadius: 18, padding: '6px 16px', cursor: 'pointer', fontSize: 12, color: u.isFollowing ? 'rgba(255,255,255,0.5)' : accent, fontFamily: 'inherit', fontWeight: 700, flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(255,255,255,0.2)', fontWeight: 700, marginBottom: 4, textTransform: 'uppercase' }}>Following {filteredFriends.length}</div>
                    {filteredFriends.map((f, i) => (
                      <button key={f.user_id || i} onClick={() => setViewingProfile(f.user_id)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '12px 14px', width: '100%', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
                        <div style={{ width: 46, height: 46, borderRadius: '50%', background: `${accent}20`, border: `1px solid ${accent}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, fontWeight: 700, color: accent, flexShrink: 0, overflow: 'hidden' }}>
                          {f.avatar_url ? <img src={f.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (f.username || 'U')[0].toUpperCase()}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 2 }}>{f.display_name || f.username || 'User'}</div>
                          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>@{f.username || 'user'} · {f.watchlistCount || 0} in watchlist{f.topGenres && f.topGenres.length > 0 ? ` · ${f.topGenres.slice(0, 2).join(', ')}` : ''}</div>
                        </div>
                        <span onClick={(e) => { e.stopPropagation(); handleFollow(f); }} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 20, padding: '5px 14px', cursor: 'pointer', fontSize: 11, color: 'rgba(255,255,255,0.5)', fontFamily: 'inherit', fontWeight: 600, flexShrink: 0 }}>
                          Following
                        </span>
                      </button>
                    ))}
        ...
        {tab === 'discover' && (
          <div style={{ padding: '16px' }}>
            ...
            {!searching && !searchQ && (
              <div>
                <div style={{ textAlign: 'center', padding: '24px 20px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 64, height: 64, borderRadius: '50%', background: `${accent}15`, border: `1px solid ${accent}30`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><SvgIcon name="search" size={24} color={accent} /></div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 28, marginBottom: 10 }}>
                  <div style={{ fontSize: 10, letterSpacing: 2, color: 'rgba(255,255,255,0.25)', fontWeight: 700, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <SvgIcon name="trophy" size={12} color={accent} /> Top Watchlists
                  </div>
                </div>
                {loadingLeaders ? (
                  <div style={{ display: 'flex', justifyContent: 'center', padding: 24 }}>
                  ...
                  <div style={{ textAlign: 'center', padding: '16px 0', fontSize: 12, color: 'rgba(255,255,255,0.25)' }}>No watchlists saved yet — be the first!</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {leaders.map((u) => (
                      <button key={u.user_id} onClick={() => setViewingProfile(u.user_id)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '10px 14px', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', width: '100%', boxSizing: 'border-box' }}>
                        <div style={{ width: 22, textAlign: 'center', fontSize: u.rank <= 3 ? 15 : 13, fontWeight: 800, color: u.rank === 1 ? '#FFD66B' : u.rank === 2 ? '#D8D8E0' : u.rank === 3 ? '#E0A468' : 'rgba(255,255,255,0.3)', flexShrink: 0 }}>{u.rank}</div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: `${accent}14`, border: `1px solid ${accent}30`, borderRadius: 20, padding: '3px 10px', flexShrink: 0 }}>
                ...
                {searchRes.map((u, i) => (
                  <div key={u.user_id || i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '12px 14px' }}>
                    <button onClick={() => setViewingProfile(u.user_id)} style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0, background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left', fontFamily: 'inherit' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 700, color: '#fff', marginBottom: 2 }}>{u.display_name || u.username || 'User'}</div>
                        <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>@{u.username || 'user'}</div>
                      </div>
                    </button>
                    {isSignedIn ? (
                      <button onClick={() => handleFollow(u)}
                        style={{ background: u.isFollowing ? 'rgba(255,255,255,0.06)' : accent, border: `1px solid ${u.isFollowing ? 'rgba(255,255,255,0.1)' : accent}`, borderRadius: 20, padding: '6px 16px', cursor: 'pointer', fontSize: 12, color: u.isFollowing ? 'rgba(255,255,255,0.5)' : '#07070F', fontFamily: 'inherit', fontWeight: 700, transition: 'all 0.2s ease', flexShrink: 0 }}>
                        {u.isFollowing ? 'Following' : 'Follow'}
                      </button>
                    ) : (
                      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', flexShrink: 0 }}>Sign in to follow</span>
                    )}
                  </div>
                ))}
              </div>
