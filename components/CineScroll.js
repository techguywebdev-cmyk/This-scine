...
export function UserProfileSheet({userId,onClose,accent,onWatchTrailer,onAddToWatchlist}){
  ...
  const handleMessageRequest=async()=>{
    if(!profile)return;
    if(!currentUser){showToast('Sign in to send a request');return;}
    if(currentUser.id===userId)return;
    try{
      const res=await fetch('/api/notifications',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({targetId:userId,type:'message_request'})});
      const data=await res.json().catch(()=>({}));
      if(!res.ok){showToast('Could not send request');return;}
      showToast(data.alreadySent?'Request already sent':`Request sent to @${profile.username}`);
...
function NotificationsPanel({onClose,accent,notifications,loading,onMarkRead,onFollowBack}){
  ...
  const notifIcon=(type)=>{
    ...
    if(type==='message_request')return{icon:'chat',color:'#7BC8FF'};
                ...
                <div key={n.id||i} onClick={()=>!n.read&&onMarkRead(n.id)} style={{display:'flex',gap:12,alignItems:'center',padding:'12px 0',borderBottom:i<notifications.length-1?`1px solid ${T.hairline}`:'none',cursor:n.read?'default':'pointer',opacity:n.read?0.5:1,transition:'opacity 0.2s ease'}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,color:T.text,lineHeight:1.4}}>
                      ...
                      {n.type==='follow'?'started following you':n.type==='follow_request'?'requested to follow you':n.type==='message_request'?'sent you a message request':n.type==='like'?'liked your review':'sent a notification'}
                    </div>
