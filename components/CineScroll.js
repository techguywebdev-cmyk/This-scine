...
const SvgIcon = ({ name, size = 20, color = 'currentColor', filled = false }) => {
  const icons = {
    ...
    sort:     ['M3 6h18','M7 12h10','M11 18h6'],
...
const MoodIcon = ({ mood, size = 24, color = '#fff' }) => {
  const icons = {
    ...
    Epic: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14.5 2.5l-5 5-3-1-4 4 2 1-3 4h4l1 3 4-3 1 2 4-4-1-3 5-5z" fill={color} fillOpacity="0.15"/><path d="M14.5 2.5l-5 5-3-1-4 4 2 1-3 4h4l1 3 4-3 1 2 4-4-1-3 5-5z"/><circle cx="14" cy="10" r="1.5" fill={color}/></svg>,
  ...
  return(
    <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
      ...
      <div style={{display:'flex',gap:5}}>
        {providers.map(p=>(<div key={p.provider_id} style={{width:24,height:24,borderRadius:6,overflow:'hidden',border:'1px solid rgba(255,255,255,0.15)',flexShrink:0}} title={p.provider_name}><img src={`https://image.tmdb.org/t/p/original${p.logo_path}`} alt={p.provider_name} style={{width:'100%',height:'100%',objectFit:'cover'}}/></div>))}
      </div>
    </div>
  );
}
...
function TrailerPlayer({movie,onClose}){
  ...
  useEffect(()=>{
    ...
    fetch(`https://api.themoviedb.org/3/${movie.mediaType||'movie'}/${movie.id}/videos?api_key=${TMDB_KEY}`)
      .then(r=>r.json()).then(data=>{const videos=data.results||[];const trailer=videos.find(v=>v.type==='Trailer'&&v.site==='YouTube')||videos.find(v=>v.type==='Teaser'&&v.site==='YouTube')||videos.find(v=>v.site==='YouTube');if(trailer)setTrailerKey(trailer.key);else setNotFound(true);setLoading(false);}).catch(()=>{setNotFound(true);setLoading(false);});
  },[movie,TMDB_KEY]);
  return(
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:95,background:'rgba(0,0,0,0.97)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',animation:'fadeIn 0.25s ease'}}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{position:'absolute',top:0,left:0,right:0,padding:'20px 20px 0',display:'flex',justifyContent:'space-between',alignItems:'flex-start',zIndex:10}}>
        <div><div style={{fontSize:11,letterSpacing:3,color:'rgba(255,255,255,0.35)',fontWeight:700,marginBottom:4,textTransform:'uppercase'}}>Official Trailer</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:20,fontWeight:900,fontStyle:'italic',color:'#fff'}}>{movie?.title}</div></div>
        <button onClick={onClose} style={{background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:'50%',width:36,height:36,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><SvgIcon name="close" size={15} color="rgba(255,255,255,0.7)"/></button>
      </div>
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxWidth:720}}>
        ...
        {!loading&&notFound&&<div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:16,padding:60}}><div style={{fontSize:40}}>🎬</div><div style={{textAlign:'center'}}><div style={{fontSize:16,fontWeight:600,color:'rgba(255,255,255,0.6)',marginBottom:6}}>No trailer available</div></div></div>}
...
function MoodScreen({onClose,onMoodSelect,accent}){
  ...
  const [timeGreeting,setTimeGreeting]=useState('tonight');

  useEffect(()=>{
    const h=new Date().getHours();
    if(h>=5&&h<12)setTimeGreeting('this morning');
    else if(h>=12&&h<17)setTimeGreeting('this afternoon');
    else if(h>=17&&h<21)setTimeGreeting('this evening');
    else setTimeGreeting('tonight');
  },[]);
  ...
  const handleTextMood=async()=>{
    ...
    const keywords={Inspired:['triumph','courage','motivat','inspir','uplifting','hope','success'],Thrilled:['thrill','action','suspense','tension','excitement','fast','adrenaline'],Scared:['scar','horror','creep','ghost','monster','dark','terrif'],Romantic:['love','romanc','date','relationship','couple','sweet','kiss'],['Mind-blown']:['mind','twist','complex','intellect','thought','sci-fi','confus','brain'],Laugh:['laugh','fun','comedy','funny','humor','comic','silly'],Emotional:['cry','sad','emotion','moving','tears','beautiful','feel'],Epic:['epic','adventure','grand','hero','battle','quest','fantasy'],Dark:['dark','crime','noir','gritty','serious','bleak','moral'],Nostalgic:['nostalg','classic','old','vintage','retro','childhood','memory']};
    ...
    const mood=FEEL_MOODS.find(m=>m.label===best)||FEEL_MOODS[0];
    handleSelect(mood);
  };

  const moodBgs={
    Inspired:'https://image.tmdb.org/t/p/w780/mDfJG3LC3Dqb67AZ52x3Z0jU0uB.jpg',
    Thrilled:'https://image.tmdb.org/t/p/w780/qNBAXBIQlnOThrVvA6mA2B5ggV6.jpg',
    Scared:'https://image.tmdb.org/t/p/w780/xazWoLealQwEgqZ89MLZklLZD3k.jpg',
    Romantic:'https://image.tmdb.org/t/p/w780/yFihWxQcmqcaBR31QM6Y8gT6aYV.jpg',
    'Mind-blown':'https://image.tmdb.org/t/p/w780/8Gxv8gSFCU0XGDykEGv7zR1n2ua.jpg',
    Laugh:'https://image.tmdb.org/t/p/w780/7IiTTgloJzvGI1TAYymCfbfl3vT.jpg',
    Emotional:'https://image.tmdb.org/t/p/w780/gEU2QniE6E77NI6lCU6MxlNBvIx.jpg',
    Epic:'https://image.tmdb.org/t/p/w780/2u7zbn8EudG6kLlBzUYqP8RyFU4.jpg',
    Dark:'https://image.tmdb.org/t/p/w780/eFMHFX0EopqOdwGqJTg1vJ2EsON.jpg',
    Nostalgic:'https://image.tmdb.org/t/p/w780/qJ2tW6WMQbzezjXI3LCOedfRGGV.jpg',
  };

  // Hero background — cinematic still
  ...
  return(
    <div style={{position:'fixed',inset:0,zIndex:90,background:'linear-gradient(160deg,#07070F 0%,#0A0A18 60%,#060610 100%)',overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none',animation:'moodFadeIn 0.3s ease'}}>
      ...
      <div style={{position:'relative',minHeight:360,display:'flex',flexDirection:'column',justifyContent:'flex-end',overflow:'hidden'}}>
        ...
        {/* Overlays */}
        <div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,rgba(7,7,15,0.5) 0%,rgba(7,7,15,0.2) 30%,rgba(7,7,15,0.8) 70%,rgba(7,7,15,1) 100%)'}}/>
        <div style={{position:'absolute',inset:0,background:'linear-gradient(to right,rgba(7,7,15,0.7) 0%,transparent 60%)'}}/>
        ...
        <div style={{position:'relative',zIndex:1,padding:'52px 20px 28px'}}>
          ...
          <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:'clamp(28px,8vw,46px)',fontWeight:900,color:'#fff',margin:'0 0 10px',lineHeight:1.05,letterSpacing:'-0.5px',maxWidth:460,textShadow:'0 2px 20px rgba(0,0,0,0.5)'}}>
            How do you want<br/>to feel {timeGreeting}?
          </h1>
          <p style={{fontSize:14,color:'rgba(255,255,255,0.55)',margin:'0 0 22px',lineHeight:1.6,maxWidth:340}}>
            Pick a mood or describe what you&apos;re in the mood for.
          </p>
            ...
            {showInput?(
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <div style={{flex:1,position:'relative'}}>
                  <input autoFocus value={moodInput} onChange={e=>setMoodInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleTextMood()} placeholder="e.g. something that makes me cry, an epic adventure..." style={{width:'100%',boxSizing:'border-box',background:'rgba(0,0,0,0.5)',backdropFilter:'blur(12px)',border:`1px solid ${accent}55`,borderRadius:14,padding:'12px 16px',color:'#fff',fontSize:14,outline:'none',fontFamily:'inherit'}}/>
                </div>
                <button onClick={handleTextMood} style={{background:accent,border:'none',borderRadius:12,width:44,height:44,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#07070F" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                </button>
          ...
          {/* Surprise Me + social proof */}
          <div style={{display:'flex',alignItems:'center',gap:14,flexWrap:'wrap'}}>
            <button onClick={handleSurprise} disabled={loading} style={{display:'flex',alignItems:'center',gap:8,background:`linear-gradient(135deg,${accent},${accent}bb)`,border:'none',borderRadius:28,padding:'12px 22px',cursor:'pointer',fontFamily:"'DM Sans',sans-serif",fontSize:14,fontWeight:700,color:'#07070F',boxShadow:`0 4px 18px ${accent}35`,transition:'all 0.2s ease',opacity:loading?0.7:1}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              <div style={{display:'flex'}}>
                {[{bg:'linear-gradient(135deg,#F5C842,#E8A020)',e:'🎬'},{bg:'linear-gradient(135deg,#B07FEF,#7040C0)',e:'🎭'},{bg:'linear-gradient(135deg,#4DA8FF,#2060C0)',e:'🎞'},{bg:'linear-gradient(135deg,#FF6BAE,#C04070)',e:'📽'}].map((a,i)=>(
                  <div key={i} style={{width:28,height:28,borderRadius:'50%',background:a.bg,border:'2px solid rgba(7,7,15,0.8)',marginLeft:i>0?-9:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,zIndex:4-i,position:'relative',boxShadow:'0 2px 8px rgba(0,0,0,0.5)'}}>{a.e}</div>
                ))}
              </div>
              <div>
                <div style={{fontSize:12,fontWeight:600,color:'rgba(255,255,255,0.9)'}}>Join 12K+ film lovers</div>
      ...
      <div style={{padding:'16px 16px 20px'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(100%,155px),1fr))',gap:10}}>
          {FEEL_MOODS.map((mood,i)=>{
            ...
            const bg=moodBgs[mood.label];
            return(
              <button key={mood.label} className="mood-btn" onClick={()=>handleSelect(mood)} disabled={loading} style={{position:'relative',borderRadius:18,border:`1px solid ${isActive?mood.color+'55':'rgba(255,255,255,0.07)'}`,padding:0,cursor:'pointer',overflow:'hidden',minHeight:160,opacity:loading&&!isActive?0.4:1,animation:`cardIn 0.45s ease ${i*0.04}s both`,boxShadow:isActive?`0 0 28px ${mood.color}30,0 8px 24px rgba(0,0,0,0.5)`:'0 2px 10px rgba(0,0,0,0.3)',transition:'box-shadow 0.2s ease,border-color 0.2s ease,transform 0.15s ease',display:'flex',flexDirection:'column',fontFamily:'inherit',textAlign:'left',background:mood.bg}}>
                {/* Background image */}
                {bg&&<div style={{position:'absolute',inset:0,backgroundImage:`url(${bg})`,backgroundSize:'cover',backgroundPosition:'center',opacity:0.28,transition:'opacity 0.3s ease'}}/>}
                {/* Gradient overlay */}
                ...
                <div style={{position:'relative',zIndex:1,padding:'16px 14px 40px',display:'flex',flexDirection:'column',gap:0,height:'100%'}}>
                  <div style={{width:44,height:44,borderRadius:12,background:`${mood.color}22`,border:`1px solid ${mood.color}40`,display:'flex',alignItems:'center',justifyContent:'center',marginBottom:12,flexShrink:0,boxShadow:`0 4px 14px ${mood.color}25`}}>
                    <MoodIcon mood={mood.label} size={22} color={mood.color}/>
                  </div>
                ...
                {/* Arrow */}
                <div style={{position:'absolute',right:12,bottom:12,width:28,height:28,borderRadius:'50%',border:`1px solid ${isActive?mood.color+'66':'rgba(255,255,255,0.12)'}`,background:isActive?`${mood.color}18`:'rgba(0,0,0,0.2)',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)',transition:'all 0.2s ease'}}>
                ...
                {/* Bottom accent */}
                <div style={{position:'absolute',bottom:0,left:'15%',right:'15%',height:2,borderRadius:1,background:`linear-gradient(to right,transparent,${mood.color},transparent)`,opacity:isActive?0.8:0.25,transition:'opacity 0.2s ease'}}/>
              </button>
            );
          })}
        </div>
      ...
      <div style={{padding:'0 16px 48px'}}>
        <div style={{background:'rgba(255,255,255,0.025)',backdropFilter:'blur(12px)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:20,padding:'22px 16px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}>
            {[
              {icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="1.5" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>,title:'Pick your mood',desc:'Choose how you want to feel.'},
              {icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="1.5" strokeLinecap="round"><path d="M12 2l2 6h6l-5 4 2 6-5-4-5 4 2-6-5-4h6l2-6z"/></svg>,title:'We do the magic',desc:'Films curated to your vibe.'},
              {icon:<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={accent} strokeWidth="1.5" strokeLinecap="round"><circle cx="12" cy="12" r="9"/><path d="M10 8l6 4-6 4V8z" fill={accent} fillOpacity="0.3"/></svg>,title:'Press play & enjoy',desc:'The perfect scene awaits.'},
            ].map((s,i)=>(
...
function ProfileSheet({onClose,accent,watchlist,setWatchlist,userReviews,loadingData}){
  ...
  const [watchlistFilter,setWatchlistFilter]=useState('all'); // all, movies, tv
  const [watchlistSort,setWatchlistSort]=useState('date'); // date, rating, title
    ...
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:100,background:'rgba(0,0,0,0.8)',backdropFilter:'blur(16px)',display:'flex',alignItems:'flex-end',animation:'fadeIn 0.2s ease'}}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',height:'92%',background:'rgba(8,8,16,0.99)',borderRadius:'24px 24px 0 0',border:'1px solid rgba(255,255,255,0.08)',borderBottom:'none',display:'flex',flexDirection:'column',animation:'sheetUp 0.35s cubic-bezier(0.22,1,0.36,1)'}}>
        ...
        <div style={{padding:'16px 20px 0',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
          <span style={{fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:900,fontStyle:'italic',color:'#fff'}}>My Profile</span>
          <div style={{display:'flex',gap:8,alignItems:'center'}}>
            <button style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,width:34,height:34,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <SvgIcon name="settings" size={15} color="rgba(255,255,255,0.4)"/>
            </button>
            <button onClick={onClose} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:10,width:34,height:34,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="close" size={15} color="rgba(255,255,255,0.4)"/></button>
          </div>
        </div>
        ...
        <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none'}}>
          ...
          {tab==='profile'&&(
            <div style={{padding:'20px'}}>
              ...
              {/* User card */}
              <div style={{background:`linear-gradient(135deg, rgba(255,255,255,0.04), rgba(255,255,255,0.02))`,border:'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:'20px',marginBottom:16,display:'flex',alignItems:'center',gap:16}}>
                <div style={{width:72,height:72,borderRadius:'50%',background:`linear-gradient(135deg,${accent}44,${accent}22)`,border:`2px solid ${accent}66`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,overflow:'hidden',boxShadow:`0 0 20px ${accent}20`}}>
                  {user?.imageUrl?<img src={user.imageUrl} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:<span style={{fontSize:28,fontWeight:700,color:accent,fontFamily:"'Playfair Display',serif"}}>{(user?.firstName||user?.username||'?')[0].toUpperCase()}</span>}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:20,fontWeight:700,color:'#fff',fontFamily:"'Playfair Display',serif",fontStyle:'italic',marginBottom:3}}>{user?.firstName||user?.username||'Cinephile'}</div>
                  <div style={{fontSize:12,color:'rgba(255,255,255,0.35)',marginBottom:8}}>{user?.primaryEmailAddress?.emailAddress}</div>
                  <div style={{display:'flex',alignItems:'center',gap:6,background:`${accent}14`,border:`1px solid ${accent}30`,borderRadius:20,padding:'3px 10px',width:'fit-content'}}>
              ...
              {/* CineScore */}
              <div style={{background:`linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))`,border:`1px solid ${accent}20`,borderRadius:20,padding:'20px',marginBottom:16}}>
                <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:16}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:13,color:'rgba(255,255,255,0.6)',lineHeight:1.5,marginBottom:10}}>{cineScore<100?'Just getting started!':cineScore<300?'Casual viewer. Keep exploring.':cineScore<600?'Dedicated cinephile.':'Elite film connoisseur. 🏆'}</div>
                ...
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8}}>
                  {[{label:'TITLES',value:saved,icon:'📚'},{label:'WATCHED',value:watched,icon:'👁'},{label:'REVIEWS',value:reviews,icon:'✍️'}].map(s=>(
                    <div key={s.label} style={{background:'rgba(255,255,255,0.04)',borderRadius:12,padding:'12px 8px',textAlign:'center'}}>
                      ...
                      <div style={{fontSize:22,fontWeight:800,color:accent,fontFamily:"'Playfair Display',serif",lineHeight:1}}>{s.value}</div>
                      <div style={{fontSize:9,letterSpacing:1.5,color:'rgba(255,255,255,0.3)',fontWeight:700,marginTop:3}}>{s.label}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:16}}>
                {[{label:'Avg Rating',value:avgRating,icon:'star'},{label:'Genres Explored',value:Object.keys(topGenres).length,icon:'gem'}].map(s=>(
                  <div key={s.label} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'14px'}}>
                ...
                <div style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'16px',marginBottom:16}}>
                  ...
                  {sortedGenres.map(([genre,count])=>(
                    <div key={genre} style={{marginBottom:10}}>
                      <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}><span style={{fontSize:13,color:'rgba(255,255,255,0.7)',fontWeight:500}}>{genre}</span><span style={{fontSize:12,color:'rgba(255,255,255,0.3)'}}>{count} films</span></div>
          ...
          {/* ── WATCHLIST TAB — Redesigned ── */}
          {tab==='watchlist'&&(
            <div>
              {/* Stats bar */}
              <div style={{margin:'16px 16px 0',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:16,padding:'14px 16px',display:'flex',alignItems:'center',gap:0}}>
                <div style={{flex:1,textAlign:'center',borderRight:'1px solid rgba(255,255,255,0.06)'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginBottom:3}}><div style={{width:22,height:22,background:'rgba(255,255,255,0.06)',borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="list" size={11} color="rgba(255,255,255,0.5)"/></div><span style={{fontSize:20,fontWeight:800,color:'#fff',fontFamily:"'Playfair Display',serif"}}>{watchlist.length}</span></div>
                  <div style={{fontSize:9,letterSpacing:1.5,color:'rgba(255,255,255,0.3)',fontWeight:700}}>TITLES</div>
                </div>
                <div style={{flex:1,textAlign:'center',borderRight:'1px solid rgba(255,255,255,0.06)'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginBottom:3}}><div style={{width:22,height:22,background:`${accent}20`,borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="star" size={11} color={accent} filled/></div><span style={{fontSize:20,fontWeight:800,color:accent,fontFamily:"'Playfair Display',serif"}}>{avgRating}</span></div>
                  <div style={{fontSize:9,letterSpacing:1.5,color:'rgba(255,255,255,0.3)',fontWeight:700}}>AVG RATING</div>
                </div>
                <div style={{flex:1,textAlign:'center'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginBottom:3}}><div style={{width:22,height:22,background:'rgba(100,180,255,0.15)',borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="check" size={11} color="#7BC8FF"/></div><span style={{fontSize:20,fontWeight:800,color:'#7BC8FF',fontFamily:"'Playfair Display',serif"}}>{watched}</span></div>
                  <div style={{fontSize:9,letterSpacing:1.5,color:'rgba(255,255,255,0.3)',fontWeight:700}}>WATCHED</div>
                </div>
              </div>

              {/* Search + filter bar */}
              <div style={{padding:'12px 16px 8px',display:'flex',gap:8,flexDirection:'column'}}>
                {/* Search */}
                <div style={{position:'relative'}}>
                  ...
                  <input value={watchlistSearch} onChange={e=>setWatchlistSearch(e.target.value)} placeholder="Search watchlist..." style={{width:'100%',boxSizing:'border-box',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:'10px 12px 10px 36px',color:'#fff',fontSize:13,outline:'none',fontFamily:'inherit'}}/>
                </div>
                ...
                <div style={{display:'flex',gap:6,alignItems:'center'}}>
                  ...
                  <div style={{display:'flex',gap:4,flex:1}}>
                    {[['all','All'],['movies','Movies'],['tv','TV Shows']].map(([val,label])=>(
                      <button key={val} onClick={()=>setWatchlistFilter(val)} style={{flex:1,background:watchlistFilter===val?accent:'rgba(255,255,255,0.04)',border:`1px solid ${watchlistFilter===val?accent:'rgba(255,255,255,0.07)'}`,borderRadius:20,padding:'6px 8px',cursor:'pointer',fontSize:11,fontWeight:watchlistFilter===val?700:400,color:watchlistFilter===val?'#07070F':'rgba(255,255,255,0.4)',fontFamily:'inherit',transition:'all 0.2s ease'}}>{label}</button>
                    ))}
                  </div>
                  {/* Sort */}
                  <select value={watchlistSort} onChange={e=>setWatchlistSort(e.target.value)} style={{background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:'6px 10px',color:'rgba(255,255,255,0.5)',fontSize:11,outline:'none',fontFamily:'inherit',cursor:'pointer'}}>
                ...
                {/* Share button */}
                {watchlist.length>0&&(
                  <button onClick={async()=>{setSharing(true);try{const d=await generateShareCard('watchlist',{name:user?.firstName||user?.username||'Cinephile',total:watchlist.length,watched,items:watchlist.map(m=>({title:m.title,year:m.year,rating:m.rating,watched:m.watched,poster:m.poster,genre:m.genre}))},accent);await shareImage(d,'My Watchlist','Check out my CineScroll watchlist!');}catch(e){console.error(e);}setSharing(false);}} disabled={sharing} style={{width:'100%',background:'rgba(255,255,255,0.04)',border:`1px solid ${accent}33`,borderRadius:12,padding:'10px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8,fontFamily:'inherit',opacity:sharing?0.6:1}}>
                    {sharing?<span style={{animation:'spin 0.8s linear infinite',display:'inline-block'}}>◌</span>:<SvgIcon name="share" size={14} color={accent}/>}
                    <span style={{fontSize:12,color:accent,fontWeight:600}}>{sharing?'Preparing…':'Share Watchlist'}</span>
                  </button>
                )}
              </div>
              ...
              {/* Watchlist items */}
              {loadingData?(
                <div style={{textAlign:'center',padding:40,color:'rgba(255,255,255,0.3)',fontSize:13,display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
                  <div style={{width:24,height:24,border:`2px solid rgba(255,255,255,0.1)`,borderTop:`2px solid ${accent}`,borderRadius:'50%',animation:'spin 0.7s linear infinite'}}/>Loading…
                </div>
              ):filteredWatchlist.length===0?(
                <div style={{textAlign:'center',padding:'32px 20px',display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
                  <div style={{width:56,height:56,borderRadius:'50%',background:'rgba(255,255,255,0.04)',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="bookmark" size={24} color="rgba(255,255,255,0.15)"/></div>
                  <div style={{fontSize:15,color:'rgba(255,255,255,0.3)',fontWeight:500}}>{watchlistSearch?'No matches found':'Your watchlist is empty'}</div>
                  <div style={{fontSize:13,color:'rgba(255,255,255,0.2)'}}>{watchlistSearch?'Try a different search term':'Tap Save on any film to add it here'}</div>
                </div>
              ):(
                <div style={{display:'flex',flexDirection:'column',gap:0,padding:'0 16px 16px'}}>
                  {filteredWatchlist.map((m,i)=>(
                    <div key={m.movie_id} style={{display:'flex',gap:12,alignItems:'flex-start',padding:'14px 0',borderBottom:'1px solid rgba(255,255,255,0.04)'}}>
                      {/* Rank number */}
                      <div style={{width:22,height:22,borderRadius:6,background:i<3?`${accent}20`:'rgba(255,255,255,0.04)',border:`1px solid ${i<3?accent+'44':'rgba(255,255,255,0.07)'}`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,marginTop:2}}>
                        <span style={{fontSize:10,fontWeight:800,color:i<3?accent:'rgba(255,255,255,0.3)'}}>{i+1}</span>
                      </div>
                      {/* Poster */}
                      <div style={{width:52,height:72,borderRadius:10,flexShrink:0,overflow:'hidden',background:m.gradient||GRADS[i%GRADS.length],position:'relative'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:'flex',alignItems:'flex-start',gap:6,marginBottom:4}}>
                          ...
                          {m.is_tv&&<span style={{fontSize:9,color:'#7BC8FF',border:'1px solid #7BC8FF44',borderRadius:4,padding:'1px 5px',flexShrink:0,marginTop:2,fontWeight:700}}>TV</span>}
                        </div>
                        <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:6,flexWrap:'wrap'}}>
                          <span style={{fontSize:11,color:'rgba(255,255,255,0.28)'}}>{m.year}</span>
                          <span style={{display:'flex',alignItems:'center',gap:2}}><SvgIcon name="star" size={10} color={accent} filled/><span style={{fontSize:11,color:accent,fontWeight:600}}>{m.rating}</span></span>
                          {m.watched&&<span style={{fontSize:9,color:accent,background:`${accent}15`,border:`1px solid ${accent}30`,borderRadius:10,padding:'1px 7px',fontWeight:700}}>Watched</span>}
                        </div>
                        {/* Genre pills */}
                        {m.genre&&m.genre.length>0&&(
                          <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:8}}>
                            {m.genre.slice(0,3).map(g=><span key={g} style={{fontSize:9,color:'rgba(255,255,255,0.35)',background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:'2px 7px'}}>{g}</span>)}
                          </div>
                        )}
                        {/* Actions */}
                        <div style={{display:'flex',gap:6,alignItems:'center'}}>
                          <button onClick={()=>toggleWatched(m)} style={{display:'flex',alignItems:'center',gap:4,background:m.watched?`${accent}15`:'rgba(255,255,255,0.04)',border:`1px solid ${m.watched?accent+'44':'rgba(255,255,255,0.07)'}`,borderRadius:20,padding:'4px 10px',cursor:'pointer',fontSize:10,color:m.watched?accent:'rgba(255,255,255,0.4)',fontFamily:'inherit',fontWeight:600}}>
                            <SvgIcon name="check" size={10} color={m.watched?accent:'rgba(255,255,255,0.4)'}/>
                            {m.watched?'Watched':'Mark watched'}
                          </button>
                          <button onClick={()=>removeFromWatchlist(m)} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:20,padding:'4px 8px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="trash" size={11} color="rgba(255,255,255,0.25)"/></button>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {/* Add more CTA */}
                  <div style={{display:'flex',alignItems:'center',gap:12,padding:'16px 0',border:'1px dashed rgba(255,255,255,0.08)',borderRadius:14,justifyContent:'center',marginTop:8}}>
                    <div style={{width:32,height:32,borderRadius:'50%',background:'rgba(255,255,255,0.04)',display:'flex',alignItems:'center',justifyContent:'center'}}><SvgIcon name="plus" size={16} color="rgba(255,255,255,0.3)"/></div>
                    <div><div style={{fontSize:13,color:'rgba(255,255,255,0.4)',fontWeight:500}}>Add something to your watchlist</div><div style={{fontSize:11,color:'rgba(255,255,255,0.2)'}}>Find your next great watch</div></div>
                  </div>
                </div>
              )}
            </div>
          ...
          {tab==='reviews'&&(
            <div style={{padding:'16px 20px'}}>
              ...
              :(<div style={{display:'flex',flexDirection:'column',gap:10}}>
                {userReviews.map(r=>(
                  <div key={r.id} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:16,padding:'16px'}}>
                    ...
                    {r.rating>0&&<div style={{display:'flex',gap:2,marginBottom:8}}>{[1,2,3,4,5].map(s=><SvgIcon key={s} name="star" size={12} color={s<=r.rating?accent:'rgba(255,255,255,0.12)'} filled={s<=r.rating}/>)}</div>}
                    <p style={{fontSize:13.5,color:'rgba(255,255,255,0.6)',lineHeight:1.6,margin:0}}>{r.text}</p>
                  </div>
                ))}
              </div>)}
            </div>
...
function CommentPanel({movie,onClose,accent,onAuthRequired}){
  ...
  const[comments,setComments]=useState([{id:1,user:'reelcritic',avatar:'R',text:'One of the defining films of the decade. Absolutely unforgettable.',likes:84,time:'2h',liked:false,replies:[]},{id:2,user:'filmbuff_mx',avatar:'F',text:'Slow burn but worth every second. That ending hit different.',likes:31,time:'5h',liked:false,replies:[]},{id:3,user:'popcorn.wav',avatar:'P',text:'Gorgeous visually but emotionally hollow.',likes:12,time:'1d',liked:false,replies:[]}]);
  ...
  const post=async()=>{
    if(!isSignedIn){onAuthRequired();return;}if(!input.trim())return;
    const username=user?.username||user?.firstName||'you';const avatar=(user?.firstName||user?.username||'Y')[0].toUpperCase();
    ...
    <div style={{position:'relative',width:'100%',height:'100%',overflow:'hidden',background:'#04040A',userSelect:'none',WebkitUserSelect:'none'}} onMouseDown={onPressStart} onMouseUp={onPressEnd} onMouseLeave={onPressEnd} onTouchStart={onPressStart} onTouchEnd={onPressEnd} onTouchCancel={onPressEnd}>
      ...
      {showHint&&isActive&&(
        <div style={{position:'absolute',top:'42%',left:'50%',transform:'translate(-50%,-50%)',zIndex:15,display:'flex',flexDirection:'column',alignItems:'center',gap:10,pointerEvents:'none',animation:'hintIn 0.4s ease'}}>
          <div style={{width:64,height:64,borderRadius:'50%',background:'rgba(0,0,0,0.65)',backdropFilter:'blur(10px)',border:`2px solid ${accent}55`,display:'flex',alignItems:'center',justifyContent:'center',boxShadow:`0 0 28px ${accent}25`}}><SvgIcon name="play" size={24} color={accent} filled/></div>
          <div style={{background:'rgba(0,0,0,0.65)',backdropFilter:'blur(10px)',borderRadius:20,padding:'5px 14px',border:'1px solid rgba(255,255,255,0.1)'}}><span style={{fontSize:11,color:'rgba(255,255,255,0.65)',fontWeight:600,letterSpacing:0.5}}>Hold for trailer</span></div>
      ...
      <div style={{position:'absolute',top:80,left:0,right:0,zIndex:10,padding:'0 16px',display:'flex',justifyContent:'space-between',alignItems:'center',opacity:isActive?1:0.5,transition:'opacity 0.4s ease'}}>
        <div style={{display:'flex',alignItems:'center',gap:7,background:'rgba(0,0,0,0.45)',backdropFilter:'blur(12px)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:20,padding:'5px 12px'}}><div style={{width:5,height:5,borderRadius:'50%',background:accent,boxShadow:`0 0 6px ${accent}`}}/><span style={{fontSize:11,fontWeight:700,color:'rgba(255,255,255,0.6)',letterSpacing:1}}>{String(index+1).padStart(2,'0')}</span>{movie.isTV&&<span style={{fontSize:9,color:accent,fontWeight:700,letterSpacing:1,marginLeft:2}}>TV</span>}</div>
        <div style={{display:'flex',alignItems:'center',gap:5,background:'rgba(0,0,0,0.45)',backdropFilter:'blur(12px)',border:`1px solid ${accent}35`,borderRadius:20,padding:'5px 12px'}}><SvgIcon name="star" size={11} color={accent} filled/><span style={{fontSize:13,fontWeight:800,color:'#fff',letterSpacing:-0.3}}>{movie.rating}</span><span style={{fontSize:10,color:'rgba(255,255,255,0.28)'}}>/10</span></div>
      </div>
...
export default function CineScroll(){
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      try{const[wRes,rRes]=await Promise.all([fetch('/api/watchlist'),fetch('/api/reviews')]);const[wData,rData]=await Promise.all([wRes.json(),rRes.json()]);const items=wData.items||[];setWatchlist(items);setWatchlistIds(new Set(items.map(m=>m.movie_id)));setUserReviews(rData.items||[]);}
      catch(e){console.error(e);}
  ...
  // ── Improved search — searches TMDB directly ──
  useEffect(()=>{
    if(!searchQ.trim()){setSearchRes([]);return;}
    const t=setTimeout(async()=>{
      setSearching(true);
      try{
        const res=await fetch(`/api/movies?search=${encodeURIComponent(searchQ)}`);
  ...
  useEffect(()=>{
    ...
    const fn=()=>{
      ...
      setActiveIndex(idx);
      setMovies(prev=>{
        [idx+1,idx+2].forEach(i=>{if(prev[i]?.backdrop){const img=new Image();img.src=prev[i].backdrop;}if(prev[i]?.poster){const img=new Image();img.src=prev[i].poster;}});
      ...
      {showSearch&&(
        <div style={{position:'fixed',inset:0,zIndex:45,background:'rgba(4,4,10,0.97)',backdropFilter:'blur(24px)',padding:'78px 16px 20px',display:'flex',flexDirection:'column',gap:12,animation:'fadeIn 0.2s ease'}}>
          ...
          <div style={{flex:1,overflowY:'auto',WebkitOverflowScrolling:'touch',display:'flex',flexDirection:'column',gap:6,scrollbarWidth:'none'}}>
            ...
            {!searching&&searchQ&&searchRes.length===0&&<div style={{textAlign:'center',padding:30,color:'rgba(255,255,255,0.3)',fontSize:13}}>No results found for &quot;{searchQ}&quot;</div>}
            {!searching&&!searchQ&&(
              <div style={{padding:'8px 0'}}>
                <div style={{fontSize:10,letterSpacing:2,color:'rgba(255,255,255,0.2)',fontWeight:700,marginBottom:10,textTransform:'uppercase'}}>Currently in feed</div>
                {movies.slice(0,8).map((m,i)=>(
                  <button key={m.id} onClick={()=>{scrollTo(i);setShowSearch(false);}} style={{display:'flex',gap:12,alignItems:'center',background:'transparent',border:'none',borderBottom:'1px solid rgba(255,255,255,0.04)',padding:'10px 0',cursor:'pointer',textAlign:'left',fontFamily:'inherit',width:'100%'}}>
                    <div style={{width:36,height:50,borderRadius:8,flexShrink:0,overflow:'hidden',background:m.gradient||GRADS[i%GRADS.length]}}>{m.poster&&<img src={m.poster} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>}</div>
                    <div>
                      <div style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,0.8)',fontFamily:"'Playfair Display',serif",fontStyle:'italic',marginBottom:2}}>{m.title}</div>
                      <div style={{fontSize:11,color:'rgba(255,255,255,0.28)',display:'flex',alignItems:'center',gap:5}}><span>{m.year}</span><span>·</span><SvgIcon name="star" size={10} color={m.accent} filled/><span style={{color:m.accent,fontWeight:600}}>{m.rating}</span>{m.isTV&&<span style={{color:'rgba(255,255,255,0.3)'}}>· TV</span>}</div>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {!searching&&searchRes.length>0&&(
              <>
                ...
                {searchRes.map((m,i)=>(
                  <button key={m.id} onClick={()=>{setMovies(p=>[m,...p.filter(x=>x.id!==m.id)]);scrollTo(0);setShowSearch(false);setSearchQ('');setSearchRes([]);}} style={{display:'flex',gap:12,alignItems:'center',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:14,padding:'11px 14px',cursor:'pointer',textAlign:'left',fontFamily:'inherit'}}>
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:3}}><span style={{fontSize:14,fontWeight:700,color:'#fff',fontFamily:"'Playfair Display',serif",fontStyle:'italic'}}>{m.title}</span>{m.isTV&&<span style={{fontSize:9,color:'#7BC8FF',border:'1px solid #7BC8FF44',borderRadius:3,padding:'1px 5px',fontWeight:700}}>TV</span>}</div>
                      <div style={{fontSize:11,color:'rgba(255,255,255,0.28)',display:'flex',alignItems:'center',gap:5,marginBottom:4}}><span>{m.year}</span><span>·</span><SvgIcon name="star" size={10} color={m.accent} filled/><span style={{color:m.accent,fontWeight:600}}>{m.rating}</span></div>
                      {m.genre&&m.genre.length>0&&<div style={{display:'flex',gap:4}}>{m.genre.slice(0,2).map(g=><span key={g} style={{fontSize:9,color:'rgba(255,255,255,0.3)',background:'rgba(255,255,255,0.05)',borderRadius:20,padding:'2px 6px'}}>{g}</span>)}</div>}
                    </div>
                    <div style={{flexShrink:0,opacity:0.4}}><SvgIcon name="play" size={14} color="#fff" filled/></div>
                  </button>
                ))}
              </>
            )}
