...
export default function CineScroll(){
  ...
  const fetchMovies=useCallback(async(mood,genre,search='',page=1,append=false,provider='')=>{
    ...
    try{
      ...
      if(append){setMovies(p=>[...p,...batch]);}
        ...
        // Prefetch page 2 immediately so first bottom reach is already warm
        if(batch.length>0){
          setTimeout(()=>{
            if(loadingMoreRef.current)return;
            pageRef.current=2;
            fetchMovies(mood,genre,search,2,true,provider);
          },400);
        }
      }
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      <div ref={containerRef} style={{position:'fixed',inset:0,overflowY:'scroll',scrollSnapType:'y proximity',WebkitOverflowScrolling:'touch',scrollbarWidth:'none',msOverflowStyle:'none',overscrollBehaviorY:'contain'}}>
        ...
        {loading?(
          ...
          <>
          {movies.map((m,i)=>(
            <div key={`${m.id}-${i}`} style={{width:'100%',height:'100dvh',scrollSnapAlign:'start',scrollSnapStop:'normal',position:'relative',flexShrink:0}}>
