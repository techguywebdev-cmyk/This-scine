...
if(type==='arc_complete')return{icon:'flame',label:'Finished a Cine Arc',color:'#FF7A2F'};
  ...
  return(
    <>
    {chatPeer&&<ChatWidget peer={chatPeer} onClose={()=>setChatPeer(null)} accent={accentColor||accent}/>}
...
function MessagesInbox({ onClose, accent, onOpenChat, onOpenProfile }) {
  ...
  const [filter, setFilter] = useState('all'); // all | unread | read | requests | spam
  ...
  const filtered = conversations.filter((c) => {
    ...
    const isRequest = !!c.is_request;
    if (filter === 'spam') return isSpam;
    if (isSpam && filter !== 'spam') return false;
    if (filter === 'unread') return !!c.unread;
    if (filter === 'read') return !c.unread;
  ...
  return (
    <>
      ...
      >
        <style>{`@keyframes sheetUp{from{transform:translateY(100%);opacity:0}to{transform:translateY(0);opacity:1}}@keyframes fadeIn{from{opacity:0}to{opacity:1}}`}</style>
        <div style={{ width: 32, height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.14)', margin: '12px auto 0', flexShrink: 0 }} />

        {/* Header: search | title | settings */}
        <div style={{ padding: '10px 14px 6px', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          {iconBtn(() => { setShowSearch((v) => !v); setShowSettings(false); }, showSearch,
            <SvgIcon name="search" size={15} color={showSearch ? accent : T.text2} />, 'Search')}
          <div style={{ flex: 1, textAlign: 'center', fontFamily: T.serif, fontSize: 20, fontWeight: 800, fontStyle: 'italic', color: T.text }}>
            Messages
          </div>
          {iconBtn(() => { setShowSettings((v) => !v); setShowSearch(false); }, showSettings,
            <SvgIcon name="settings" size={15} color={showSettings ? accent : T.text2} />, 'Filter')}
          {iconBtn(onClose, false, <SvgIcon name="close" size={14} color={T.text2} />, 'Close')}
        </div>
        ...
        {showSettings && (
          <div style={{ padding: '4px 14px 12px', flexShrink: 0, display: 'flex', gap: 6, overflowX: 'auto' }}>
            {[
              { id: 'all', label: 'All' },
              ...
              <button
                ...
                style={{
                  background: filter === f.id ? `${accent}22` : T.surface2,
                  border: `1px solid ${filter === f.id ? accent + '55' : T.hairline}`,
                  borderRadius: 14,
                  padding: '7px 12px',
                  fontSize: 12,
                  fontWeight: 700,
                  color: filter === f.id ? accent : T.text2,
                  cursor: 'pointer',
                  fontFamily: 'inherit',
                  whiteSpace: 'nowrap',
                  flexShrink: 0,
        ...
        {/* Avatar strip — centered row of peers (profile on tap) */}
        {!loading && conversations.length > 0 && filter === 'all' && !searchQ && (
          <div
            style={{
              display: 'flex',
              justifyContent: 'center',
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: '50%',
                    padding: 0,
                    border: c.unread ? `2px solid ${dot}` : `1px solid ${T.hairline}`,
                    background: T.surface2,
                    overflow: 'hidden',
                    cursor: 'pointer',
                    flexShrink: 0,
                ...
                >
                  {c.avatar_url ? (
                    ...
                    <span style={{ display: 'flex', width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', fontFamily: T.serif, fontWeight: 700, fontSize: 15, color: T.text2 }}>
                      {(c.display_name || c.username || 'U')[0].toUpperCase()}
                    </span>
                  )}
                  {c.unread && (
                    <span style={{ position: 'absolute', bottom: 0, right: 0, width: 8, height: 8, borderRadius: '50%', background: dot, border: `1.5px solid ${T.bg}` }} />
                  )}
                </button>
              );
            })}
        ...
        <div style={{ flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: 40 }}>
              <div style={{ fontSize: 13, color: T.text3, marginBottom: 10 }}>{error}</div>
              <button type="button" onClick={load} style={{ background: `${accent}18`, border: `1px solid ${accent}40`, borderRadius: 14, padding: '8px 14px', cursor: 'pointer', fontSize: 12, color: accent, fontFamily: 'inherit', fontWeight: 600 }}>Retry</button>
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 24px', color: T.text3, fontSize: 13 }}>
              {filter === 'spam' ? 'No flagged conversations' : filter === 'unread' ? 'No unread messages' : filter === 'requests' ? 'No message requests' : 'No conversations yet'}
            </div>
          ) : (
            filtered.map((c, i) => {
              const dot = CHAT_UNREAD_DOTS[i % CHAT_UNREAD_DOTS.length];
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      overflow: 'hidden',
                      background: T.surface2,
                      border: isUnread ? `2px solid ${dot}` : `1px solid ${T.hairline}`,
                      padding: 0,
                      cursor: 'pointer',
                      flexShrink: 0,
                      display: 'flex',
                  ...
                  >
                    {c.avatar_url ? (
                      ...
                      <span style={{ fontFamily: T.serif, fontWeight: 700, fontSize: 16, color: T.text2 }}>
                        {(c.display_name || c.username || 'U')[0].toUpperCase()}
                      </span>
                    )}
                  </button>

                  {/* Rest of row → open chat (no username label) */}
                  <button
                    type="button"
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 3 }}>
                      <span style={{
                        ...
                        {c.from_me ? 'You: ' : ''}{c.last_text || 'Open chat'}
                      </span>
                      <span style={{ fontSize: 11, color: isUnread ? dot : T.text3, flexShrink: 0, fontWeight: isUnread ? 600 : 400 }}>
                        {timeAgo(c.last_at)}
                      </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 11, color: T.text3 }}>
                        {isSpam ? 'Spam' : c.is_request ? 'Request' : isUnread ? 'Unread' : 'Tap to open'}
                      </span>
                      {isUnread && (
                        <span style={{ width: 8, height: 8, borderRadius: '50%', background: dot, boxShadow: `0 0 8px ${dot}88`, marginLeft: 'auto' }} />
                      )}
                    </div>
                  </button>
                  ...
                  <button
                    ...
                    title={isSpam ? 'Unflag spam' : 'Flag spam'}
                    style={{
                      background: 'none',
                      border: 'none',
                      color: isSpam ? '#E50914' : T.text3,
                      cursor: 'pointer',
                      fontSize: 11,
                      fontWeight: 600,
                      padding: '4px 6px',
...
function ChatWidget({ peer, onClose, accent }) {
  ...
  const send = async () => {
    ...
    setInput('');
    setShowStickers(false);
    playChatSound(CHAT_THEMES[chatThemeId] || CHAT_THEMES.classic);
    await postMessage(
      { text, msg_type: 'text' },
      {
        id: `tmp-${Date.now()}`,
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {peerInfo?.avatar_url ? (
              ...
              <span style={{ fontSize: 15, fontWeight: 700, color: themeAccent, fontFamily: theme.font }}>{(name || 'U')[0].toUpperCase()}</span>
            )}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: theme.text, fontFamily: theme.font, fontStyle: 'italic' }}>{name}</div>
          ...
          <button
            ...
            title="Chat theme"
            style={{
              background: showChatSettings ? `${themeAccent}22` : 'rgba(255,255,255,0.05)',
              border: `1px solid ${showChatSettings ? themeAccent + '55' : 'rgba(255,255,255,0.08)'}`,
              borderRadius: '50%',
              width: 34,
              height: 34,
              cursor: 'pointer',
          ...
          >
            <SvgIcon name="settings" size={14} color={showChatSettings ? themeAccent : 'rgba(255,255,255,0.5)'} />
          </button>
        </div>
        ...
        {showChatSettings && (
          ...
          >
            <div style={{ fontSize: 11, fontWeight: 700, color: theme.textMuted, letterSpacing: 0.6, textTransform: 'uppercase', marginBottom: 10 }}>
              Chat theme
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {Object.values(CHAT_THEMES).map((th) => (
                <button
                  key={th.id}
                  type="button"
                  onClick={() => { playChatSound(th); applyTheme(th.id); }}
                  style={{
                    background: th.bg,
                    border: chatThemeId === th.id ? `2px solid ${th.accent}` : '1px solid rgba(255,255,255,0.1)',
                    borderRadius: 12,
                    padding: '10px 4px',
                    cursor: 'pointer',
                    fontFamily: th.font,
                ...
                >
                  ...
                  <div style={{ width: 14, height: 14, borderRadius: th.radius > 12 ? '50%' : 3, background: th.accent }} />
                  <span style={{ fontSize: 10, fontWeight: 700, color: th.text, fontStyle: 'italic' }}>{th.label}</span>
                </button>
              ))}
            </div>
            <div style={{ marginTop: 8, fontSize: 10, color: theme.textMuted, textAlign: 'center' }}>
              Themes change color, type, bubble shape & send sound
            </div>
          </div>
        )}

        {error && (
          <div style={{ padding: '8px 16px', background: 'rgba(255,80,80,0.1)', borderBottom: `1px solid ${T.hairline}`, flexShrink: 0 }}>
        ...
        >
          {loading ? (
            <div style={{ textAlign: 'center', padding: 28, color: T.text3, fontSize: 13 }}>Loading…</div>
          ) : messages.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '36px 20px', color: T.text3, fontSize: 13, lineHeight: 1.5 }}>
              Say hi — stickers, voice notes, and text all work here.
            </div>
          ) : (
            messages.map((m) => {
              const isSticker = m.msg_type === 'sticker' || (m.text && [...m.text].length <= 2 && m.text.trim().length > 0 && !/[a-zA-Z0-9]/.test(m.text));
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    maxWidth: isSticker ? '70%' : '78%',
                    display: 'flex',
                    flexDirection: 'column',
                    alignItems: m.from_me ? 'flex-end' : 'flex-start',
                  }}
                >
                  {isVoice && m.media_url ? (
                    <div
                      style={{
                        background: m.from_me ? theme.bubbleMe : theme.bubbleThem,
                        borderRadius: m.from_me ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                        padding: '10px 12px',
                        border: `1px solid ${themeAccent}22`,
                        minWidth: 180,
                      }}
                    >
                      <audio controls src={m.media_url} style={{ width: '100%', height: 36, outline: 'none' }} />
                    </div>
                  ) : isGif && m.media_url ? (
                    <div
                      style={{
                    ...
                    <div style={{ fontSize: 42, lineHeight: 1.1, padding: '4px 2px' }}>{m.text}</div>
                  ) : (
                    <div
                      style={{
                        background: m.from_me ? theme.bubbleMe : theme.bubbleThem,
                        color: theme.text,
                        borderRadius: m.from_me ? `${theme.radius}px ${theme.radius}px 4px ${theme.radius}px` : `${theme.radius}px ${theme.radius}px ${theme.radius}px 4px`,
                        padding: '10px 13px',
                        fontSize: 13.5,
                        lineHeight: 1.45,
                        fontWeight: 500,
                        fontFamily: theme.bodyFont,
                        border: m.from_me ? 'none' : `1px solid ${T.hairline}`,
                        whiteSpace: 'pre-wrap',
                  ...
                  >
                    {m.from_me && (
                      <span style={{ color: m.read ? accent : 'rgba(255,255,255,0.35)', fontWeight: 700, letterSpacing: -1, fontSize: 11 }}>
                        {m.read ? '✓✓' : (m.delivered !== false ? '✓✓' : '✓')}
                      </span>
                    )}
                    <span style={{ color: m.from_me && m.read ? accent : T.text3 }}>{statusLabel(m)}</span>
                  </div>
                </div>
              );
            })
          ...
          >
            <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
              {[
                ...
                <button
                  ...
                  style={{
                    background: mediaTab === tab.id ? 'rgba(255,255,255,0.1)' : 'transparent',
                    border: `1px solid ${mediaTab === tab.id ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.08)'}`,
                    borderRadius: 20,
                    padding: '7px 14px',
                    fontSize: 12,
                    fontWeight: 700,
                    color: mediaTab === tab.id ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.4)',
                    cursor: 'pointer',
                    fontFamily: 'inherit',
                  }}
                >
            ...
            {mediaTab === 'stickers' ? (
                  ...
                  {Object.keys(STICKER_PACKS).map((pack) => (
                    <button
                      ...
                      style={{
                        ...
                        border: `1px solid ${stickerPack === pack ? 'rgba(255,255,255,0.22)' : 'rgba(255,255,255,0.08)'}`,
                        borderRadius: 14,
                        padding: '6px 12px',
                        fontSize: 11,
                        fontWeight: 700,
                        color: stickerPack === pack ? 'rgba(255,255,255,0.9)' : 'rgba(255,255,255,0.45)',
                        cursor: 'pointer',
                        fontFamily: 'inherit',
                        whiteSpace: 'nowrap',
                        flexShrink: 0,
              ...
              <>
                ...
                {gifsLoading ? (
                  <div style={{ textAlign: 'center', padding: 20, color: 'rgba(255,255,255,0.35)', fontSize: 12 }}>Loading GIFs…</div>
                ) : gifs.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 20, color: 'rgba(255,255,255,0.35)', fontSize: 12 }}>Search for a reaction or movie moment</div>
                ) : (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, overflowY: 'auto', maxHeight: 180 }}>
                    {gifs.map((g) => (
                      <button
                        key={g.id}
                        type="button"
...
{recording ? (
          ...
          >
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 3, marginTop: 6, alignItems: 'flex-end', height: 18 }}>
                {[4, 10, 7, 14, 6, 12, 8, 16, 5, 11, 9, 13, 6, 10, 8].map((h, i) => (
                  <div
                    key={i}
                    style={{
                      width: 3,
                      ...
                      animation: `pulseRec ${0.6 + (i % 5) * 0.1}s ease infinite`,
          ...
          >
            ...
            <button
              ...
              onClick={() => {
                setShowStickers((v) => !v);
                if (!showStickers) setMediaTab('stickers');
              }}
              style={{
                width: 40,
                height: 40,
                borderRadius: '50%',
                background: showStickers ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.08)',
                border: '1px solid rgba(255,255,255,0.1)',
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
            ...
            {/* pill input with mic / send inside */}
            <div
              style={{
                flex: 1,
                display: 'flex',
            ...
            >
              ...
              {input.trim() ? (
                ...
                <button
                  ...
                  title="Voice note"
                >
                  <SvgIcon name="mic" size={18} color="rgba(255,255,255,0.55)" />
                </button>
              )}
            </div>
          </div>
...
function FriendsScreen({ onClose, accent, onWatchTrailer, onAddToWatchlist }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 90, background: T.bg, display: 'flex', flexDirection: 'column', animation: 'playerSlideUp 0.4s cubic-bezier(0.22,1,0.36,1)' }}>
      ...
      {chatPeer && <ChatWidget peer={chatPeer} onClose={() => setChatPeer(null)} accent={accent} />}
      {showMessages && (
        <MessagesInbox
          onClose={() => setShowMessages(false)}
          accent={accent}
          onOpenChat={(p) => { setShowMessages(false); setChatPeer(p); }}
