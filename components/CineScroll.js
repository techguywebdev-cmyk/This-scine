  ...
  const post = () => {
    ...
    if (replyingTo) {
      setComments(p => p.map(c => c.id === replyingTo.id
        ? {...c, replies: [...(c.replies||[]), {id:Date.now(),user:'you',avatar:'Y',text:input,likes:0,time:'now',liked:false}]}
        : c
      ));
    } else {
      setComments(p => [{id:Date.now(),user:'you',avatar:'Y',text:input,likes:0,time:'now',liked:false,replies:[]},...p]);
    }
    setInput('');
    setReplyingTo(null);
  };
...
function MovieCard({ movie, isActive, index, onFindSimilar }) {
        ...
        <div style={{marginBottom:26}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {MOODS.map(m => {
              ...
              return (
                <button key={m.label} onClick={() => {onMood(m.label);onClose();}} style={{display:'flex',alignItems:'center',gap:10,background:on?'rgba(255,255,255,0.09)':'rgba(255,255,255,0.03)',border:`1px solid ${on?'rgba(255,255,255,0.22)':'rgba(255,255,255,0.06)'}`,borderRadius:14,padding:'13px 15px',cursor:'pointer',fontFamily:'inherit',transition:'all 0.2s ease'}}>
...
export default function CineScroll() {
  ...
  useEffect(()=>{
    ...
    const fn=()=>{
      ...
      setActiveIndex(idx);
      setMovies(prev=>{
        if(idx>=prev.length-3&&!loadingMoreRef.current){
          pageRef.current+=1;
  ...
  const handleSimilarSelect=(movie)=>{
    setMovies(p=>[movie,...p]);
  ...
  return(
    <div style={{width:'100%',height:'100vh',background:'#04040A',overflow:'hidden',position:'relative',fontFamily:"'DM Sans',sans-serif",color:'#fff'}}>
        ...
        {loading?(
          ...
          movies.map((m,i)=>(
            <div key={`${m.id}-${i}`} style={{width:'100%',height:'100vh',scrollSnapAlign:'start',scrollSnapStop:'always',position:'relative'}}>
              <MovieCard movie={m} isActive={i===activeIndex} index={i} onFindSimilar={setSimilarMovie}/>
            </div>
          ))
        )}
      </div>
      ...
      {similarMovie&&(
        <SimilarSheet movie={similarMovie} onClose={()=>setSimilarMovie(null)} accent={accent} onSelect={handleSimilarSelect}/>
      )}
