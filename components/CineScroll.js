...
function CoverCropModal({file,onCancel,onSave,accent}){
  const ASPECT=2.2; // width:height ratio of the exported cover crop
  ...
  const handleSave=()=>{
    ...
    setSaving(true);
    const OUT_W=1200;const OUT_H=Math.round(OUT_W/ASPECT);
  ...
  return(
    <div style={{position:'fixed',inset:0,zIndex:200,background:'#05050a',display:'flex',flexDirection:'column'}}>
      <div style={{padding:'16px 16px 12px',display:'flex',justifyContent:'space-between',alignItems:'center',flexShrink:0}}>
        ...
        <span style={{fontSize:14,fontWeight:700,color:'#fff'}}>Adjust Cover Photo</span>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'0 16px'}}>
        <div ref={containerRef} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp}
          style={{width:'100%',maxWidth:480,aspectRatio:`${ASPECT}`,position:'relative',overflow:'hidden',borderRadius:16,border:`1px solid ${accent}44`,background:'#000',touchAction:'none',cursor:'grab'}}>
          {imgUrl&&(
...
function ProfileSheet({onClose,accent,watchlist,setWatchlist,userReviews,loadingData}){
  ...
  useEffect(()=>{
    fetch('/api/settings').then(r=>r.json()).then(d=>{setWatchlistPublic(d.watchlist_public!==false);setBio(d.bio||'');setBioInput(d.bio||'');setCoverUrl(d.cover_url||null);setLoadingSettings(false);}).catch(()=>setLoadingSettings(false));
  },[]);
...
// USER PROFILE SHEET
function UserProfileSheet({userId,onClose,accent,onWatchTrailer}){
                ...
                loadingActivity?(
                  ...
                  <div style={{display:'flex',flexDirection:'column'}}>
                    {activity.map((item,i)=>{
                      ...
                      return(
                        <button key={item.id||i} onClick={()=>item.movie_id&&onWatchTrailer&&onWatchTrailer({id:item.movie_id,title:item.movie_title,poster:item.movie_poster,year:item.movie_year,rating:item.movie_rating,accent:item.movie_accent||accentColor,mediaType:item.is_tv?'tv':'movie'})}
...
// COMMENT PANEL
function CommentPanel({movie,onClose,accent,onAuthRequired,onWatchTrailer}){
  const{isSignedIn,user}=useUser();
  const[comments,setComments]=useState([]);
  const[loading,setLoading]=useState(true);
    ...
    try{
      ...
      if(data.comment){
        if(parentId){
          ...
          fetch('/api/activity',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'reviewed',movieId:movie?.id,movieTitle:movie?.title,moviePoster:movie?.poster,movieYear:movie?.year,movieRating:movie?.rating,movieAccent:movie?.accent,username:user?.username||user?.firstName||'user',avatarUrl:user?.imageUrl||null})}).catch(()=>{});
        }
      }
    }catch{}
  };
  ...
  return(
    <>
    {viewingProfile&&<UserProfileSheet userId={viewingProfile} onClose={()=>setViewingProfile(null)} accent={accent} onWatchTrailer={onWatchTrailer}/>}
...
function MovieCard({movie,isActive,index,onFindSimilar,onAuthRequired,onSave,isSaved,onTrailer}){
  ...
  return(
    <div style={{position:'relative',width:'100%',height:'100%',overflow:'hidden',background:'#04040A',userSelect:'none',WebkitUserSelect:'none'}} onMouseDown={onPressStart} onMouseUp={onPressEnd} onMouseLeave={onPressEnd} onTouchStart={onPressStart} onTouchEnd={onPressEnd} onTouchCancel={onPressEnd}>
      ...
      {showComments&&<CommentPanel movie={movie} onClose={()=>setShowComments(false)} accent={accent} onAuthRequired={()=>{setShowComments(false);onAuthRequired();}} onWatchTrailer={onTrailer}/>}
                    ...
                    {filteredFriends.map((f, i) => (
                      <button key={f.user_id || i} onClick={() => setViewingProfile(f.user_id)} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 14, padding: '12px 14px', width: '100%', textAlign: 'left', cursor: 'pointer', fontFamily: 'inherit' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{f.watchlistCount || 0} in watchlist{f.topGenres && f.topGenres.length > 0 ? ` · ${f.topGenres.slice(0, 2).join(', ')}` : ''}</div>
                        </div>
...
export default function CineScroll(){
  ...
  const{openSignIn}=useClerk();
  const[movies,setMovies]=useState([]);const[loading,setLoading]=useState(true);const[activeIndex,setActiveIndex]=useState(0);const[activeGenre,setActiveGenre]=useState('');const[activeMood,setActiveMood]=useState('Trending');const[showFilter,setShowFilter]=useState(false);const[showSearch,setShowSearch]=useState(false);const[showAuth,setShowAuth]=useState(false);const[showProfile,setShowProfile]=useState(false);const[showMood,setShowMood]=useState(false);const[showFriends,setShowFriends]=useState(false);const[trailerMovie,setTrailerMovie]=useState(null);const[searchQ,setSearchQ]=useState('');const[searchRes,setSearchRes]=useState([]);const[searching,setSearching]=useState(false);const[similarMovie,setSimilarMovie]=useState(null);const[watchlistIds,setWatchlistIds]=useState(new Set());const[watchlist,setWatchlist]=useState([]);const[userReviews,setUserReviews]=useState([]);const[loadingProfileData,setLoadingProfileData]=useState(false);
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      <div style={{position:'fixed',top:0,left:0,right:0,zIndex:40,padding:'18px 16px 0',background:'linear-gradient(to bottom,rgba(4,4,10,0.9) 0%,transparent 100%)',display:'flex',justifyContent:'space-between',alignItems:'center',pointerEvents:'none'}}>
        ...
        <div style={{display:'flex',gap:8,pointerEvents:'all',alignItems:'center'}}>
          <button onClick={()=>{if(!isSignedIn){setShowAuth(true);return;}setShowFriends(true);}} style={{background:'rgba(0,0,0,0.55)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:12,width:38,height:38,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(12px)'}}>
            <SvgIcon name="people" size={17} color="rgba(255,255,255,0.7)"/>
          </button>
      ...
      {showFriends&&<FriendsScreen onClose={()=>setShowFriends(false)} accent={accent} onWatchTrailer={setTrailerMovie}/>}
