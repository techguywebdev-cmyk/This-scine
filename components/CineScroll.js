                    ...
                    <div key={row.key} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 0'}}>
                      ...
                      <button type="button" onClick={()=>toggleNotify(row.key)} disabled={loadingSettings} role="switch" aria-checked={!!notifyPrefs[row.key]} style={{width:40,height:24,borderRadius:12,border:'none',cursor:'pointer',background:notifyPrefs[row.key]?accent:'rgba(255,255,255,0.12)',position:'relative',flexShrink:0,padding:0}}>
                        <div style={{position:'absolute',top:3,left:notifyPrefs[row.key]?20:3,width:18,height:18,borderRadius:'50%',background:'#fff',transition:'left 0.2s ease'}}/>
                            ...
                            <div style={{margin:'0 0 14px',padding:'14px',borderRadius:14,border:`1px solid ${T.hairline}`,background:T.surface}}>
                <div style={{fontSize:10,color:T.text3,letterSpacing:0.6,textTransform:'uppercase',marginBottom:8}}>Data</div>
                <a href="https://www.themoviedb.org" target="_blank" rel="noopener noreferrer" style={{display:'flex',alignItems:'center',gap:10,textDecoration:'none'}}>
                  <img src="https://www.themoviedb.org/assets/2/v4/logos/v2/blue_short-8e7b30f73a4020692ccca9c88bafe5dcb6f8a62a4c6bc55cd9ba82bb2cd95f6c.svg" alt="TMDB" style={{height:14,opacity:0.85}}/>
                </a>
                <div style={{fontSize:11,color:T.text3,lineHeight:1.45,marginTop:8}}>
