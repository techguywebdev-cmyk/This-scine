    ...
    <div style={{position:'fixed',inset:0,zIndex:90,background:'linear-gradient(160deg,#07070F 0%,#0A0A18 60%,#060610 100%)',overflowY:'auto',WebkitOverflowScrolling:'touch',scrollbarWidth:'none',animation:'moodFadeIn 0.3s ease'}}>
      ...
      <div style={{position:'relative',minHeight:340,display:'flex',flexDirection:'column',justifyContent:'flex-end',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1,padding:'52px 20px 28px'}}>
          <Eyebrow color={accent} style={{marginBottom:14}}>Mood Discovery</Eyebrow>
          <h1 style={{fontFamily:T.serif,fontSize:'clamp(28px,8vw,44px)',fontWeight:800,fontStyle:'italic',color:T.text,margin:'0 0 10px',lineHeight:1.08,letterSpacing:'-0.3px',maxWidth:460,textShadow:'0 2px 20px rgba(0,0,0,0.5)'}}>How do you want<br/>to feel {timeGreeting}?</h1>
          <p style={{fontSize:14,color:T.text2,margin:'0 0 22px',lineHeight:1.6,maxWidth:340}}>Pick a mood or describe what you are in the mood for.</p>
          <div style={{marginBottom:22}}>
            {showInput?(
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <input autoFocus value={moodInput} onChange={e=>setMoodInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleTextMood()} placeholder="e.g. something that makes me cry..." style={{flex:1,background:'rgba(0,0,0,0.5)',backdropFilter:'blur(12px)',border:`1px solid ${accent}50`,borderRadius:14,padding:'12px 16px',color:T.text,fontSize:14,outline:'none',fontFamily:'inherit'}}/>
                ...
                <button onClick={()=>{setShowInput(false);setMoodInput('');}} style={{background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:12,width:44,height:44,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><SvgIcon name="close" size={13} color={T.text2}/></button>
              </div>
            ):(
              <button onClick={()=>setShowInput(true)} style={{display:'flex',alignItems:'center',gap:8,background:'rgba(0,0,0,0.35)',backdropFilter:'blur(12px)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:14,padding:'12px 16px',cursor:'pointer',fontFamily:'inherit',fontSize:13,color:T.text2,width:'100%',textAlign:'left',maxWidth:400}}>
                <SvgIcon name="chat" size={15} color={T.text3}/>Describe what you are in the mood for...
              </button>
            )}
          </div>
          <button onClick={handleSurprise} disabled={loading} style={{display:'flex',alignItems:'center',gap:8,background:`linear-gradient(135deg,${accent},${accent}bb)`,border:'none',borderRadius:28,padding:'12px 22px',cursor:'pointer',fontFamily:'inherit',fontSize:14,fontWeight:700,color:'#07070F',boxShadow:`0 4px 18px ${accent}35`,opacity:loading?0.7:1}}>
            ...
            return(
              <button key={mood.label} className="mood-btn" onClick={()=>handleSelect(mood)} disabled={loading} style={{position:'relative',borderRadius:18,border:`1px solid ${isActive?mood.color+'55':'rgba(255,255,255,0.07)'}`,padding:0,cursor:'pointer',overflow:'hidden',minHeight:160,opacity:loading&&!isActive?0.4:1,animation:`cardIn 0.45s ease ${i*0.04}s both`,boxShadow:isActive?`0 0 28px ${mood.color}30,0 8px 24px rgba(0,0,0,0.5)`:'0 2px 10px rgba(0,0,0,0.3)',transition:'all 0.15s ease',display:'flex',flexDirection:'column',fontFamily:'inherit',textAlign:'left',background:mood.bg}}>
                ...
                <div style={{position:'relative',zIndex:1,padding:'16px 14px 40px',display:'flex',flexDirection:'column',height:'100%'}}>
                  ...
                  <div style={{fontSize:16,fontWeight:700,color:'#fff',fontFamily:'inherit',marginBottom:5,lineHeight:1.2}}>{mood.label}</div>
      ...
      <div style={{position:'fixed',top:0,left:0,right:0,zIndex:40,padding:'18px 16px 0',background:'linear-gradient(to bottom,rgba(4,4,10,0.9) 0%,transparent 100%)',display:'flex',justifyContent:'space-between',alignItems:'center',pointerEvents:'none'}}>
        ...
        <div style={{display:'flex',gap:8,pointerEvents:'all',alignItems:'center'}}>
          ...
          <button onClick={()=>setShowMood(true)} style={{background:'rgba(0,0,0,0.55)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:12,width:38,height:38,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(12px)'}}>
            <SvgIcon name="masks" size={17} color="rgba(255,255,255,0.75)"/>
          </button>
