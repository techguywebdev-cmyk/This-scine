...
function ListsScreen({onClose,accent,onWatchTrailer,onSave,watchlistIds,onOpenList}){
  const{isSignedIn}=useUser();
...
export default function CineScroll(){
  ...
  return(
    <div style={{position:'fixed',inset:0,background:'#04040A',fontFamily:"'DM Sans',sans-serif",color:'#fff',overflow:'hidden'}}>
      ...
      {showLists&&<ListsScreen onClose={()=>setShowLists(false)} accent={accent} onWatchTrailer={setTrailerMovie} onSave={handleSave} watchlistIds={watchlistIds} onOpenList={id=>setTopLevelList(id)}/> }
