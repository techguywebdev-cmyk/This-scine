...
export async function GET(req) {
  ...
  try {
    if (withId) {
      let res = await fetch(
        `${db('messages')}?or=(and(from_user_id.eq.${userId},to_user_id.eq.${withId}),and(from_user_id.eq.${withId},to_user_id.eq.${userId}))&order=created_at.asc&limit=120&select=id,from_user_id,to_user_id,text,created_at,read,read_at,delivered,msg_type,media_url,edited_at`,
        { headers }
      );
      if (!res.ok) {
        res = await fetch(
    ...
    let res = await fetch(
      `${db('messages')}?or=(from_user_id.eq.${userId},to_user_id.eq.${userId})&order=created_at.desc&limit=200&select=id,from_user_id,to_user_id,text,created_at,read,msg_type,media_url`,
      { headers }
    );
    if (!res.ok) {
      res = await fetch(
        `${db('messages')}?or=(from_user_id.eq.${userId},to_user_id.eq.${userId})&order=created_at.desc&limit=200&select=id,from_user_id,to_user_id,text,created_at,read`,
        { headers }
      );
    }
    const rows = await res.json();
...
export async function POST(request) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (errText.includes('msg_type') || errText.includes('media_url') || errText.includes('delivered') || errText.includes('read_at') || errText.includes('edited_at')) {
...
export async function PATCH(request) {
  ...
  try {
    ...
    const existingRes = await fetch(
      `${db('messages')}?id=eq.${id}&from_user_id=eq.${userId}&select=id,from_user_id,to_user_id,text,created_at,read,read_at,delivered,msg_type,media_url,edited_at`,
      { headers }
    );
    ...
    if (existing.msg_type && existing.msg_type !== 'text' && existing.msg_type !== 'sticker') {
      return Response.json({ error: 'This message type cannot be edited' }, { status: 400 });
    }

    const editedAt = new Date().toISOString();
    let patchRes = await fetch(`${db('messages')}?id=eq.${id}&from_user_id=eq.${userId}`, {
      method: 'PATCH',
      headers,
      body: JSON.stringify({ text, edited_at: editedAt }),
    });
    if (!patchRes.ok) {
      const errText = await patchRes.text();
      if (errText.includes('edited_at')) {
        patchRes = await fetch(`${db('messages')}?id=eq.${id}&from_user_id=eq.${userId}`, {
          method: 'PATCH',
          headers,
          body: JSON.stringify({ text }),
        });
      } else {
        return Response.json({ error: `Failed to edit: ${errText.slice(0, 150)}` }, { status: 500 });
      }
    }
    if (!patchRes.ok) {
      const errText = await patchRes.text();
    ...
    return Response.json({
      message: mapMessage({ ...existing, ...row, text, edited_at: row?.edited_at || editedAt }, userId),
    });
  } catch (err) {
