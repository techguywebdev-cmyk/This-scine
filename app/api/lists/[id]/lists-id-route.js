...
// GET /api/lists/[id] - single list with movies + stats + cover
export async function GET(req, { params }) {
  ...
  try {
    ...
    if (!Array.isArray(listRows) || !listRows[0]) {
    ...
    try {
      ...
      creator.display_name = u.firstName
        ? `${u.firstName}${u.lastName ? ' ' + u.lastName : ''}`
        : creator.username;
      creator.avatar_url = u.imageUrl || null;
      const nickRes = await fetch(
        `${db('user_settings')}?user_id=eq.${list.user_id}&select=nickname`,
        { headers }
      );
    ...
    const avgRating = ratingArr.length
      ? Math.round((ratingArr.reduce((s, r) => s + r.rating, 0) / ratingArr.length) * 10) / 10
      : null;
    const isFollowing = userId
      ? (Array.isArray(follows) ? follows : []).some((r) => r.user_id === userId)
      : false;
    const viewerRating = userId
      ? ratingArr.find((r) => r.user_id === userId)?.rating || null
      : null;

    // Cover: custom cover_url, else first movie poster
    const cover_poster = list.cover_url || movieList[0]?.movie_poster || null;
    const cover_accent = movieList[0]?.movie_accent || null;

    return Response.json({
      list: {
        ...list,
        ...creator,
        movie_count: movieList.length,
        follower_count: (Array.isArray(follows) ? follows : []).length,
        avg_rating: avgRating,
        rating_count: ratingArr.length,
