...
async function enrichLists(lists, viewerId) {
  ...
  return lists.map(l=>{
    ...
    const isPublic = l.is_public !== false; // default public if column missing
    return {
      ...l,
      is_public: isPublic,
...
export async function GET(req) {
  ...
  try {
    ...
    if (tab==='mine') {
      ...
      // Public discovery — prefer is_public=true; if column missing Supabase may error, fallback without filter
      listsRes=await fetch(`${db('community_lists')}?or=(is_public.eq.true,is_public.is.null)&order=created_at.desc&limit=100`,{headers});
      if (!listsRes.ok) {
...
export async function POST(req) {
  ...
  try {
    ...
    if (!res.ok) {
      // Retry without is_public if column doesn't exist yet
      if (responseText.includes('is_public') || res.status === 400) {
        const {is_public:_, ...fallback}=payload;
        const res2=await fetch(db('community_lists'),{method:'POST',headers,body:JSON.stringify(fallback)});
        const text2=await res2.text();
        if (!res2.ok) {
          console.error('Supabase create list error:',res2.status,text2);
          return Response.json({error:`Database error: ${text2.slice(0,200)}`},{status:500});
        }
        let data2; try { data2=JSON.parse(text2); } catch { return Response.json({error:'Bad response'},{status:500}); }
        const list2=Array.isArray(data2)?data2[0]:data2;
        return Response.json({list:{...list2,is_public:true}, warning:'is_public column missing — run SQL'});
      }
