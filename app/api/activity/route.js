...
// POST /api/activity
// Movie events: { type: saved|watched|reviewed, movieId, movieTitle, ... }
// List events:  { type: list_follow, listId, listTitle, listPoster, listAccent }
export async function POST(request) {
  ...
  try {
    ...
    if (!isListFollow && !movieId) {
      return Response.json({ error: 'movieId is required for this activity type' }, { status: 400 });
    }
    if (isListFollow && !listTitle && !listId) {
      return Response.json({ error: 'listTitle or listId is required for list_follow' }, { status: 400 });
    }

    // Reuse movie_* columns for list follows so we don't need a schema migration:
    // movie_title = list title, movie_poster = cover, movie_accent = accent,
    // movie_year stores listId as a string reference when possible.
    const payload = {
      user_id: userId,
      username: username || null,
      avatar_url: avatarUrl || null,
      type,
      movie_id: isListFollow ? null : movieId,
      movie_title: isListFollow ? listTitle || 'a list' : movieTitle || null,
      movie_poster: isListFollow ? listPoster || null : moviePoster || null,
      movie_year: isListFollow ? (listId ? String(listId) : null) : movieYear || null,
      movie_rating: isListFollow ? null : movieRating || null,
      movie_accent: isListFollow ? listAccent || null : movieAccent || null,
    };
